...
(function() {
  ...
  // ========== FIX #3: DYNAMIC WALLET ACCOUNTS (always visible) ==========
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    // Fix #2 + #4: update method cards & form before anything else
    updateMethodCardsVisibility();

    // If we switched away from USDT and active method is crypto, reset to bank
  ...
  function setView(v) {
    ...
    // Extra safety for withdrawal view
    if (v === 'withdraw') {
      forceUpdateWithdrawHero();
      updateMethodCardsVisibility();
      renderWithdrawForm(activeWithdrawMethod);
  ...
  // Final safety net – render wallet accounts & force hero after everything is loaded
  setTimeout(() => {
    renderWalletAccounts();
    forceUpdateWithdrawHero();
  }, 200);
