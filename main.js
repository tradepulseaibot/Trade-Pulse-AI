// ===== Trade Pulse AI – main.js (CHART FIXED + Community hide) =====
(function() {
  ...
  // ========== CHART (FIXED: single-point & empty handling) ==========
  function buildChart() {
    ...
    // Prevent division by zero with a single data point by duplicating it
    if (chartData.length === 1) {
      chartData.push({ label: chartData[0].label, value: chartData[0].value });
    }

    let maxV = Math.max(...chartData.map(d => d.value)) * 1.1;
    if (!maxV || maxV <= 0) maxV = 1000; // fallback to avoid NaN
  ...
  // ===== FIXED: openDepositModal no longer reads the button's textContent =====
  function openDepositModal() {
    // currentCurrency is always correct – no need to parse the button
    updateDepositModalCurrency();
  ...
  function setView(v) {
    ...
    // Hide global install button on Community tab
    if (v === 'community') {
