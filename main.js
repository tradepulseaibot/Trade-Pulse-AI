// ===== Trade Pulse AI – main.js (FINAL – all fixes, per‑user flags, onboarding highlight, terms-first) =====
(function() {
    ...
    if (step.position === 'center') {
      ...
      onboardingSteps.forEach(s => {
        if (s.target) {
          ...
          if (el) { el.style.boxShadow = ''; el.style.outline = ''; el.style.zIndex = ''; el.style.position = ''; }
        }
      });
      return;
    }
  ...
  function initializeApp() {
    ...
    if (!stored) {
      // No user stored; shouldn't happen if called from auth, but fallback
      return;
    }
    ...
    const uid = stored.userId || stored.username;   // unique per account

    if (sessionStorage.getItem('justSignedUp')) {
      // New user – always show notifications and terms immediately.
      ...
      if (!localStorage.getItem('acceptedTerms_' + uid)) {
        // Show terms modal immediately – onboarding will start after acceptance
        showTermsModal();
      }
      // Do NOT start onboarding here – it will be triggered by the terms acceptance handler.

      // Clear the referral banner flag so it reappears for new account
      sessionStorage.removeItem('referralBannerShown');
      // We'll remove the justSignedUp flag in the acceptance handler
    } else {
      // Returning user – check per‑account flags
      if (!localStorage.getItem('hasSeenWelcomeNotifs_' + uid)) {
        window.addWelcomeNotifications();
        localStorage.setItem('hasSeenWelcomeNotifs_' + uid, 'true');
      }
      if (!localStorage.getItem('acceptedTerms_' + uid)) {
        showTermsModal();   // still needs to accept
      }
    }
    // =================================
  ...
  // Expose the initialiser globally
