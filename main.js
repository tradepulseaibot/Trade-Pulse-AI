// ===== Trade Pulse AI – main.js (FINAL – fully synchronized, no tab‑switch triggers) =====
(function() {
  console.log('main.js FINAL – synchronized initialization');
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    // Force method cards and method switching
    ...
    // ★ Always rebuild withdrawal form so it's ready when user navigates there
  ...
  function renderWithdrawForm(method) {
    ...
    // Force method to USDT TRC20 if currency is USDT and method is bank/ewallet
    if (currentCurrency === 'USDT') {
  ...
  function tryInitialize() {
    if (window.__appInitialized) return true; // already done
    const stored = getStoredUser();
    if (!stored) return false; // not logged in yet
    ...
    // Immediately set the correct withdrawal method for default currency
    if (currentCurrency === 'USDT') {
      setWithdrawMethod('usdt_trc20');
      updateMethodCardsVisibility(); // hide bank card
    } else {
      setWithdrawMethod('bank');
    }
    ...
    // Extra safety: rebuild wallet accounts and withdrawal form after DOM settled
    setTimeout(() => {
      renderWalletAccounts();
      forceUpdateWithdrawHero();
  ...
  // Poll every 100ms until the user appears (max 5 seconds)
  let initAttempts = 0;
  (function pollInit() {
    if (tryInitialize() || ++initAttempts > 50) return; // stop after 5s
