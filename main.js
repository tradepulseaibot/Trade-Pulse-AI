// ===== Trade Pulse AI – main.js (FINAL – currency button icon, dropdown no icon, wallet icon, payment icon, etc.) =====
(function() {
  const $ = (id) => document.getElementById(id);
  const rates = { USDT:0.000667, USD:0.00067, EUR:0.00061, GBP:0.00052, NGN:1, CAD:0.00073, JPY:0.0000067, AUD:0.00065 };
  const symbols = { USDT:'USDT', USD:'$', EUR:'€', GBP:'£', NGN:'₦', CAD:'C$', JPY:'¥', AUD:'A$' };
  ...
  // ========== PER‑USER FORCED DEMO HELPERS ==========
  function isAdminLoggedIn() {
    const user = getStoredUser();
    return user && user.isAdmin;
  }
  function getForcedDemoKey(username) { return `forcedDemo_${username}`; }
  function getForcedDemoData(username) {
    try { return JSON.parse(localStorage.getItem(getForcedDemoKey(username)) || 'null'); } catch { return null; }
  }
  function setForcedDemoData(username, data) {
    localStorage.setItem(getForcedDemoKey(username), JSON.stringify(data));
  }
  function clearForcedDemoData(username) {
    localStorage.removeItem(getForcedDemoKey(username));
  }
  function isForcedDemoActive() {
    const user = getStoredUser();
    if (!user || user.isAdmin) return false;
    const data = getForcedDemoData(user.username);
  ...
  const paymentAccounts = {
    ...
    CAD:  { bank: 'RBC Royal Bank', accountNumber: '1234567890', transitNumber: '00001', institutionNumber: '003', accountName: 'Anthony Diamond' },
    JPY:  { bank: 'MUFG Bank', accountNumber: '9876543210', branchCode: '001', accountName: 'Anthony Diamond' },
    AUD:  { bank: 'Commonwealth Bank', accountNumber: '1122334455', bsb: '062-000', accountName: 'Anthony Diamond' }
  };
  ...
  const depositPresets = {
    ...
    CAD: [50, 100, 200, 500, 1000],
    JPY: [5000, 10000, 50000, 100000, 500000],
    AUD: [50, 100, 200, 500, 1000]
  };
  ...
  const withdrawPresets = {
    ...
    CAD: [50, 100, 200, 500, 'max'],
    JPY: [5000, 10000, 50000, 100000, 'max'],
    AUD: [50, 100, 200, 500, 'max']
  };
  ...
  // ----- Icon generators -----
  // For currency button (USDT gets ₮, others get their symbol)
  function getCurrencyButtonIcon(currency) {
    const sym = currency === 'USDT' ? '₮' : (symbols[currency] || currency);
    const colors = {
      USDT: '#22c55e', USD: '#3b82f6', EUR: '#a855f7', GBP: '#f59e0b',
      NGN: '#a855f7', CAD: '#ec4899', JPY: '#f59e0b', AUD: '#3b82f6'
    };
    const fill = colors[currency] || '#22c55e';
    return `<svg viewBox="0 0 24 24" width="20" height="20"><circle cx="12" cy="12" r="10" fill="${fill}"/><text x="12" y="17" text-anchor="middle" fill="white" font-size="10" font-weight="bold">${sym}</text></svg>`;
  }

  // For dropdown and anywhere else (same as above)
  function getCurrencyIconSVG(currency) {
  ...
  // We no longer populate dropdown icons – they stay empty.
  function updateCurrencyDropdownIcons() {
    // Intentionally empty – no icons in dropdown
  }
  ...
  const bankIconMap = {
    ...
    'RBC Royal Bank': 'assets/banks/rbc.png', 'TD Canada Trust': 'assets/banks/td.png',
    'Scotiabank': 'assets/banks/scotiabank.png', 'BMO': 'assets/banks/bmo.png',
    'CIBC': 'assets/banks/cibc.png', 'MUFG Bank': 'assets/banks/mufg.png',
    'Sumitomo Mitsui': 'assets/banks/smfg.png', 'Mizuho Bank': 'assets/banks/mizuho.png',
    'Japan Post Bank': 'assets/banks/japanpost.png', 'Resona Bank': 'assets/banks/resona.png',
    'Commonwealth Bank': 'assets/banks/commbank.png', 'ANZ': 'assets/banks/anz.png',
    'Westpac': 'assets/banks/westpac.png', 'NAB': 'assets/banks/nab.png',
    'Bank of Queensland': 'assets/banks/boq.png'
  };
    ...
    if (currentCurrency === 'USDT') {
      ...
      container.innerHTML = `
        <div class="wallet-account">
          <div class="wallet-account-top-row">
            <div class="wallet-account-left">
              <div class="wallet-coin green" style="background:linear-gradient(135deg, #15803d, #22c55e); display:grid; place-items:center;">
                ${getCurrencyButtonIcon('USDT')}
              </div>
        ...
        <div class="wallet-account">
          <div class="wallet-account-top-row">
            <div class="wallet-account-left">
              <div class="wallet-coin amber" style="background:linear-gradient(135deg, #a16207, #f59e0b); display:grid; place-items:center;">
                ${getCurrencyButtonIcon('USDT')}
              </div>
              <div class="wallet-account-info">
      ...
      const symbolMap = { NGN: '₦', USD: '$', EUR: '€', GBP: '£', CAD: 'C$', JPY: '¥', AUD: 'A$' };
      const nameMap = { NGN: 'Naira Wallet', USD: 'USD Wallet', EUR: 'EUR Wallet', GBP: 'GBP Wallet', CAD: 'CAD Wallet', JPY: 'JPY Wallet', AUD: 'AUD Wallet' };
  ...
  function updateAll() {
    // No dropdown icons – intentionally empty
    updateCurrencyDropdownIcons();

    if (isForcedDemoActive()) {
      const user = getStoredUser();
      const forcedData = getForcedDemoData(user.username);
      if (forcedData) {
        if (base.homeBalance === 0) {
          const savedBalance = forcedData.balance || 20000;
          base.homeBalance = savedBalance;
          base.walletTotal = savedBalance;
          base.walletAvailable = savedBalance;
          if (!forcedData.balance) {
            forcedData.balance = 20000;
            setForcedDemoData(user.username, forcedData);
          }
        }
        base.totalInvested = forcedData.balance || 20000;
        base.totalWithdrawn = 0;
        base.activePlans = 0;
        base.walletLocked = 0;
        base.withdrawable = 0;
        base.totalProfit = base.homeBalance - (forcedData.balance || 20000);
      ...
      checkAndStopDemoGrowth();
    }

    // Currency button – icon + text
    $('currencyBtn').innerHTML = getCurrencyButtonIcon(currentCurrency) + ' ' + currentCurrency + ' ▾';
  ...
  function openPaymentModal(amount, isInvestment = false) {
    const symbol = symbols[currentCurrency];
    const icon = getCurrencyButtonIcon(currentCurrency);
    $('paymentAmount').innerHTML = `${icon} ${symbol} ${amount.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
    ...
    if (account.transitNumber) extraInfo += `<br><small style="color:#9ca3af;">Transit: ${account.transitNumber}</small>`;
    if (account.institutionNumber) extraInfo += `<br><small style="color:#9ca3af;">Institution: ${account.institutionNumber}</small>`;
    if (account.branchCode) extraInfo += `<br><small style="color:#9ca3af;">Branch Code: ${account.branchCode}</small>`;
    if (account.bsb) extraInfo += `<br><small style="color:#9ca3af;">BSB: ${account.bsb}</small>`;
  ...
  $('confirmTransferBtn')?.addEventListener('click', () => {
    if (blockIfForcedDemo()) return; // <-- guard added here
  ...
  function setView(v) {
    ...
    // === NEW: Hide global install button on Community tab ===
    if (v === 'community') {
        const globalBtn = document.getElementById('pwaInstallBtn');
        if (globalBtn) globalBtn.style.display = 'none';
    } else {
        const globalBtn = document.getElementById('pwaInstallBtn');
        // Show it only if the PWA prompt was already captured
        if (globalBtn && window.deferredPrompt) {
  ...
  // REFERRAL MODAL "HOW DOES IT WORK?" TOGGLE (bound on open)
  $('drawerReferBtn')?.addEventListener('click', () => {
    updateReferModal();
    openModal('referModal');

    // Bind toggle directly now – fixes first visit issue
  ...
  // Modal overlay click to close (including notificationPreviewModal)
  ...
  // Notification Preview Modal - close button explicit fix
  ...
  // ========== ADMIN TOGGLE UI IN DRAWER (per‑user panel) ==========
  function addAdminToggleUI() {
    const drawerPanel = $('drawerPanel');
    if (!drawerPanel) return;

    const old = document.getElementById('adminManageRow');
    if (old) old.remove();

    const row = document.createElement('div');
    row.id = 'adminManageRow';
    ...
    row.innerHTML = `
      <div class="drawer-menu-left">
        <svg class="drawer-menu-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <path d="M16 11c1.7 0 3-1.3 3-3s-1.3-3-3-3-3 1.3-3 3 1.3 3 3 3Z"/><path d="M8 13c1.7 0 3-1.3 3-3S9.7 7 8 7 5 8.3 5 10s1.3 3 3 3Z"/><path d="M3 19c.7-2.7 3-4 5-4s4.3 1.3 5 4"/>
        </svg>
        <span class="drawer-menu-label">Manage Users</span>
      </div>
      <svg class="drawer-menu-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="m9 6 6 6-6 6"/></svg>
    `;
    ...
    row.addEventListener('click', openAdminPanel);
  }
  ...
  function openAdminPanel() {
    ...
    nonAdminUsers.forEach(user => {
      const forcedData = getForcedDemoData(user.username);
      const enabled = forcedData?.enabled || false;
      const bal = forcedData?.balance || 20000;
      rowsHtml += `
        <div style="display:flex; align-items:center; gap:10px; padding:10px 0; border-bottom:1px solid rgba(255,255,255,.06);">
          <div style="flex:1; min-width:0;">
            ...
            <div style="font-size:12px; color:#9ca3af;">Real balance: ${fmt(user.homeBalance || 0)}</div>
          </div>
          <label style="position:relative; display:inline-block; width:44px; height:24px;">
            <input type="checkbox" class="admin-user-toggle" data-username="${user.username}" ${enabled ? 'checked' : ''}>
            <span style="position:absolute; cursor:pointer; top:0; left:0; right:0; bottom:0; background:${enabled ? '#a855f7' : 'rgba(255,255,255,.2)'}; border-radius:24px; transition:.3s;"></span>
          </label>
          <input type="number" class="admin-user-balance" data-username="${user.username}" value="${bal}" step="1000" min="1000" style="width:100px; height:36px; background:#0b1020; border:1px solid rgba(255,255,255,.1); color:#fff; border-radius:8px; padding:0 8px;">
        </div>
      `;
    });

    const modalDiv = document.createElement('div');
    modalDiv.id = 'adminPanelModal';
    modalDiv.className = 'modal-overlay open';
    modalDiv.innerHTML = `
      <div class="modal-panel" style="max-width:600px; max-height:80vh; overflow-y:auto;">
        <div class="modal-header">
          <div class="modal-title">Manage Users</div>
          <button class="modal-close-btn" id="closeAdminPanelBtn">✕</button>
        </div>
        <div style="margin-bottom:16px;">Toggle forced demo mode and set a starting balance for each user. Changes are saved immediately.</div>
        ${rowsHtml}
      </div>
    `;
    document.body.appendChild(modalDiv);

    document.getElementById('closeAdminPanelBtn').addEventListener('click', () => modalDiv.remove());
    modalDiv.addEventListener('click', (e) => { if (e.target === modalDiv) modalDiv.remove(); });

    // Toggle events
    modalDiv.querySelectorAll('.admin-user-toggle').forEach(toggle => {
      toggle.addEventListener('change', function() {
        ...
        if (enabled) {
          ...
          if (user) {
            const existingData = getForcedDemoData(username) || {};
            existingData.enabled = true;
            existingData.balance = existingData.balance || 20000;
            existingData.originalBalance = user.homeBalance || 0;
            existingData.demoStartTime = Date.now();
            setForcedDemoData(username, existingData);
          }
        } else {
          const forcedData = getForcedDemoData(username);
          if (forcedData) {
            const users = JSON.parse(localStorage.getItem('tradePulseUsers') || '[]');
            const user = users.find(u => u.username === username);
            if (user) {
              user.homeBalance = forcedData.originalBalance || 0;
              localStorage.setItem('tradePulseUsers', JSON.stringify(users));
              const current = getStoredUser();
              if (current && current.username === username) {
                current.homeBalance = user.homeBalance;
                saveUser(current);
              }
            }
            clearForcedDemoData(username);
          }
        }
      });
    });
    ...
    modalDiv.querySelectorAll('.admin-user-balance').forEach(input => {
      input.addEventListener('change', function() {
        const username = this.dataset.username;
        const newBalance = parseFloat(this.value) || 20000;
        const forcedData = getForcedDemoData(username) || {};
        forcedData.balance = newBalance;
        if (!forcedData.demoStartTime) forcedData.demoStartTime = Date.now();
        setForcedDemoData(username, forcedData);
      });
    });
  }
  ...
  // ========== FORCED DEMO GROWTH STOP AFTER 2 DAYS ==========
  let demoKycModalShown = false;
  function checkAndStopDemoGrowth() {
    if (!isForcedDemoActive()) return;
    const user = getStoredUser();
    const forcedData = getForcedDemoData(user.username);
    if (!forcedData || !forcedData.demoStartTime) return;
    const twoDaysMs = 2 * 24 * 60 * 60 * 1000;
    if (Date.now() - forcedData.demoStartTime >= twoDaysMs) {
      forcedData.demoGrowthStopped = true;
      setForcedDemoData(user.username, forcedData);
      if (!demoKycModalShown) {
        showKycUnverifiedModal();
        demoKycModalShown = true;
      }
    }
  }

  // ========== CHART AUTO-GROWTH (FORCED DEMO MODE – SMOOTH LIGHT UPDATE) ==========
  setInterval(() => {
    ...
    if (isForcedDemoActive()) {
      const user = getStoredUser();
      const forcedData = getForcedDemoData(user.username);
      if (!forcedData || forcedData.demoGrowthStopped) return;

      const dailyProfitRate = 0.03; // 3% per day
      const secondsInDay = 24 * 60 * 60;
      const growthPerSecond = 1 + (dailyProfitRate / secondsInDay);
      const growthPer60s = Math.pow(growthPerSecond, 60);

      const newBalance = base.homeBalance * growthPer60s;
      base.homeBalance = newBalance;
      base.walletTotal = newBalance;
      base.walletAvailable = newBalance;
      base.withdrawable = 0;
      base.totalProfit = newBalance - (forcedData.balance || 20000);
      ...
      // Light DOM update – only balance & chart
      const elHomeBalance = $('homeBalance');
      if (elHomeBalance) elHomeBalance.textContent = fmt(base.homeBalance);
      const elHomeFx = $('homeFx');
      if (elHomeFx) elHomeFx.textContent = `≈ $ ${(base.homeBalance * rates['USD']).toFixed(2)}`;
      const elTotalProfit = $('totalProfit');
      if (elTotalProfit) elTotalProfit.textContent = fmt(Math.max(0, base.totalProfit));
      buildChart(); // rebuild chart only
      return;
    }
  ...
  $('acceptTermsBtn')?.addEventListener('click', () => {
    ...
    // If this is a new sign‑up (justSignedUp flag still present), start the onboarding after a short delay
    if (sessionStorage.getItem('justSignedUp')) {
      sessionStorage.removeItem('justSignedUp');  // prevent it from being used again
      if (!localStorage.getItem('onboardingComplete_' + uid)) {
        // Start onboarding after a short delay so the dashboard settles
        setTimeout(() => startOnboarding(), 1500);
      }
    }
  });

  // ========== ONBOARDING TOUR (REDESIGNED + PER‑USER FLAG) ==========
  const onboardingSteps = [
    {
      target: null,
      title: 'Welcome to Trade Pulse AI!',
      text: 'Your smart trading dashboard for daily profits. Let\'s take a quick tour.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2" width="48" height="48"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>',
      position: 'center'
    },
    {
      target: '#homeBalance',
      title: 'Your Balance',
      text: 'This shows your total account balance. It updates as you trade and earn.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2" width="36" height="36"><path d="M21 12v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v6Z"/><path d="M16 14h4"/></svg>',
      position: 'bottom'
    },
    {
      target: '.nav-item[data-view="invest"]',
      title: 'Invest',
      text: 'Explore investment plans. Choose a plan, deposit, and earn daily profits.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2" width="36" height="36"><path d="M4 19V5"/><path d="M4 19h16"/><path d="M8 16v-6"/><path d="M12 16V8"/><path d="M16 16v-3"/></svg>',
      position: 'top'
    },
    {
      target: '.nav-item[data-view="withdraw"]',
      title: 'Withdraw',
      text: 'Cash out your profits easily. Withdrawals are processed quickly.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2" width="36" height="36"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 21h14"/></svg>',
      position: 'top'
    },
    {
      target: '.nav-item[data-view="community"]',
      title: 'Community',
      text: 'Chat with other traders, share wins, and see live withdrawal proofs.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2" width="36" height="36"><path d="M16 11c1.7 0 3-1.3 3-3s-1.3-3-3-3-3 1.3-3 3 1.3 3 3 3Z"/><path d="M8 13c1.7 0 3-1.3 3-3S9.7 7 8 7 5 8.3 5 10s1.3 3 3 3Z"/><path d="M3 19c.7-2.7 3-4 5-4s4.3 1.3 5 4"/></svg>',
      position: 'top'
    },
    {
      target: '#menuBtn',
      title: 'Menu',
      text: 'Open the drawer for your profile, security, settings, and more.',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2" width="36" height="36"><path d="M3 6h18M3 12h18M3 18h18"/></svg>',
      position: 'top'
    }
  ];
  ...
  function showOnboardingStep(index, retryCount = 0) {
    ...
    if (step.target) {
      ...
      if (!targetEl && retryCount < 15) {
        setTimeout(() => showOnboardingStep(index, retryCount + 1), 300);
        return;
      }
      if (!targetEl) {
        nextOnboardingStep();
    ...
    if (textEl) {
      textEl.innerHTML = `
        ...
        <p style="margin:0; font-size:14px; color:#cbd5e1;">${step.text}</p>
      `;
    }

    // Progress dots
    if (indicatorEl) {
      indicatorEl.innerHTML = onboardingSteps.map((_, i) =>
        `<span style="width:8px; height:8px; border-radius:50%; display:inline-block; background:${i === index ? '#a855f7' : 'rgba(255,255,255,0.3)'}; margin-right:6px;"></span>`
      ).join('');
    }

    const nextBtn = $('onboardingNextBtn');
    if (nextBtn) {
      nextBtn.textContent = index === onboardingSteps.length - 1 ? 'Got it!' : 'Next';
    }
    ...
    // ---- Target‑based steps ----
    // Smoothly scroll the target element into view, offset for bottom nav
    const bottomNavHeight = 80; // height of the bottom navigation bar
    ...
    __onboardingScrollTimer = setTimeout(() => {
      ...
      // Reset any previous inline styles that could interfere
      tooltip.style.margin = '0';
      ...
      // Arrow positioning – centered relative to the tooltip, pointing at the target
      const arrowSize = 12; // half of the arrow width
      ...
      targetEl.style.position = 'relative'; // so z-index works
      ...
      requestAnimationFrame(() => {
        tooltip.style.transition = 'opacity 0.35s ease';
  ...
  // ========== DASHBOARD GREETING (REMOVED) ==========

  // ========== GLOBAL WELCOME NOTIFICATIONS ==========
  window.addWelcomeNotifications = function() {
    addNotification('Welcome to Trade Pulse AI!', 'Your account has been created successfully. Explore the dashboard, check out the investment plans, and join our community to start your trading journey. We wish you profitable trades!', 'promo', 'assets/notifications/welcome.jpg');
    addNotification('Referral Program – Earn 5% Commission', 'Invite your friends to Trade Pulse AI! When they sign up with your referral code and deposit at least $10, you’ll earn 5% of their deposit amount. Share your code now and start earning together.', 'promo', 'assets/notifications/referral.jpg');
    addNotification('⏳ Limited Offer – 43 Spots Left!', 'Registration for our exclusive VIP Signal Group is closing soon! Only 43 spots remain. Join now to receive premium signals with a 92% win rate and priority support. Don\'t miss this opportunity.', 'promo', 'assets/notifications/limited_offer.jpg');
  };

  // ========== APP INITIALISATION (called from auth script) ==========
  function initializeApp() {
    ...
    // Load per‑user forced demo data
    if (isForcedDemoActive()) {
      const forcedData = getForcedDemoData(stored.username);
      if (forcedData) {
        base.homeBalance = forcedData.balance || 20000;
        base.walletTotal = base.homeBalance;
        base.walletAvailable = base.homeBalance;
        base.totalInvested = forcedData.balance || 20000;
        base.totalWithdrawn = 0;
        base.activePlans = 0;
        if (!forcedData.demoStartTime) {
          forcedData.demoStartTime = Date.now();
          setForcedDemoData(stored.username, forcedData);
        }
      }
    } else {
    ...
    // Removed updateDashboardGreeting call

    // ===== POST‑LOGIN TRIGGERS (PER‑USER) =====
