...
(function() {
  ...
  // SVG icon generator for currency switcher
  function getCurrencyIconSVG(currency) {
    const sym = symbols[currency] || currency;
    return `<svg viewBox="0 0 24 24" width="20" height="20"><circle cx="12" cy="12" r="10" fill="#22c55e"/><text x="12" y="17" text-anchor="middle" fill="white" font-size="10" font-weight="bold">${sym}</text></svg>`;
  }
  ...
  function updateAll() {
    ...
    // Update currency button with icon
    $('currencyBtn').innerHTML = getCurrencyIconSVG(currentCurrency) + ' ' + currentCurrency + ' ▾';
  ...
  function openPaymentModal(amount, isInvestment = false) {
    const symbol = symbols[currentCurrency];
    const iconSVG = getCurrencyIconSVG(currentCurrency);
    ...
    $('paymentModalTitle').innerHTML = `${iconSVG} ${isInvestment ? 'Investment Payment' : 'Deposit Payment'}`;
  ...
  function setView(v) {
    ...
    // Show referral banner when entering Community tab (after 7s)
    if (v === 'community') {
  ...
  function showReferralBanner() {
    ...
    banner.style.zIndex = '100'; // ensure on top
