// ===== Trade Pulse AI – main.js (FINAL with window.setView fix) =====
(function() {
  ...
  function updateDrawerUserInfo() {
    ...
    if (kycBadge) {
      ...
      // The click handler is now attached in profile.js after openKycModal is defined,
      // so we do not set onclick here to avoid race conditions.
    }
  ...
  window.setView = setView;  // <-- FIX: expose globally for profile close button (though no longer used, kept for safety)
  ...
  function tryInitialize() {
    ...
    refresh2faUI();
    updateCardDisplay();
    // Ensure the referral toggle is attached after DOM is ready (already in body end, safe)
    initReferHowToggle();
