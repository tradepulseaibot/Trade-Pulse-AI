// ===== Trade Pulse AI – main.js (DEBUG VERSION with visual debug panel) =====
(function() {
  // ========== DEBUG PANEL SETUP ==========
  ...
  function createDebugPanel() {
    ...
    panel.id = 'debugPanel';
    panel.style.cssText = `
      position:fixed; bottom:90px; right:10px; width:340px; max-height:420px;
      background:rgba(0,0,0,0.92); color:#0f0; font-family:monospace; font-size:11px;
      z-index:9999; border-radius:10px; border:1px solid #0f0;
      display:flex; flex-direction:column; overflow:hidden;
      box-shadow:0 0 10px rgba(0,255,0,0.5);
      transition:all 0.3s ease;
    `;
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 10px;background:#111;border-bottom:1px solid #0f0;">
        <span style="font-weight:bold;color:#0f0;">🐞 Debug Panel</span>
        <div>
          <button id="debugCopyBtn" style="background:#0f0;color:#000;border:none;border-radius:4px;padding:2px 8px;font-weight:bold;cursor:pointer;margin-right:4px;">Copy</button>
          <button id="debugClearBtn" style="background:#0f0;color:#000;border:none;border-radius:4px;padding:2px 8px;font-weight:bold;cursor:pointer;">Clear</button>
          <button id="debugToggleBtn" style="background:#0f0;color:#000;border:none;border-radius:4px;padding:2px 8px;font-weight:bold;cursor:pointer;">−</button>
        </div>
      </div>
      <div id="debugLogContainer" style="flex:1;overflow-y:auto;padding:6px;max-height:320px;background:#000;">
        <div style="color:#0f0;">Debug panel active…</div>
      </div>
    `;
    ...
    if (copyBtn) {
      copyBtn.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(logText).then(() => alert('Debug logs copied!')).catch(() => {
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        debugLogs = [];
        logContainer.innerHTML = '<div style="color:#0f0;">Cleared.</div>';
      });
    }
  }
  ...
  window.addEventListener('unhandledrejection', (e) => {
    debugLog(`Unhandled rejection: ${e.reason}`, 'error');
  });
  ...
  // ========== ORIGINAL APP CODE (fully instrumented) ==========
  ...
  // ========== DYNAMIC WALLET ACCOUNTS (instrumented) ==========
  function renderWalletAccounts() {
    debugLog('renderWalletAccounts called');
    let container = document.getElementById('walletAccountsContainer');
    if (!container) {
      debugLog('walletAccountsContainer not found, creating one');
      container = document.createElement('div');
      container.id = 'walletAccountsContainer';
      const walletView = document.getElementById('walletView');
      if (walletView) {
        walletView.appendChild(container);
        debugLog('appended container to walletView');
      } else {
        debugLog('walletView not found, cannot append', 'error');
        return;
      }
    }
    const totalUsdt = base.homeBalance * rates.USDT;
    debugLog(`Rendering wallets for ${currentCurrency}, homeBalance=${base.homeBalance}`);
    ...
    debugLog('wallet accounts rendered');
  }

  // ========== FORCE WITHDRAWAL HERO SYMBOL (instrumented) ==========
  function forceUpdateWithdrawHero() {
    debugLog(`forceUpdateWithdrawHero called, balanceHidden=${balanceHidden}`);
    ...
    if (!withdrawBalanceEl) { debugLog('withdrawBalance element missing', 'warn'); }
    if (!withdrawFxEl) { debugLog('withdrawFx element missing', 'warn'); }
    if (!withdrawableAmtEl) { debugLog('withdrawableAmount element missing', 'warn'); }
  ...
  // ========== UPDATE ALL (instrumented) ==========
  function updateAll() {
    debugLog(`updateAll called, currentCurrency=${currentCurrency}, balanceHidden=${balanceHidden}, homeBalance=${base.homeBalance}`);
  ...
  // ========== CURRENCY SWITCHER (instrumented) ==========
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    debugLog(`Currency changed: ${oldCurrency} → ${currentCurrency}`);
    ...
    if (currentCurrency === 'USDT') {
      if (!activeWithdrawMethod.startsWith('usdt')) {
        debugLog('Switching method to usdt_trc20');
        setWithdrawMethod('usdt_trc20');
      }
    } else {
      if (activeWithdrawMethod.startsWith('usdt')) {
        debugLog('Switching method to bank');
    ...
    renderWalletAccounts();
    debugLog('Rebuilding withdrawal form after currency change');
  ...
  // Balance Hide Toggles (all three eye icons) – instrumented
  $('homeBalanceEye')?.addEventListener('click', () => {
    balanceHidden = !balanceHidden;
    debugLog(`Dashboard eye toggled, balanceHidden=${balanceHidden}`);
    updateAll();
  });
  document.querySelector('.withdraw-eye')?.addEventListener('click', () => {
    balanceHidden = !balanceHidden;
    debugLog(`Withdraw eye toggled, balanceHidden=${balanceHidden}`);
    updateAll();
  });
  document.querySelector('.wallet-eye')?.addEventListener('click', () => {
    balanceHidden = !balanceHidden;
    debugLog(`Wallet eye toggled, balanceHidden=${balanceHidden}`);
    updateAll();
  });
  $('hideBalanceToggle')?.addEventListener('click', () => {
    balanceHidden = !balanceHidden;
    debugLog(`Hide balance toggle clicked, balanceHidden=${balanceHidden}`);
    updateAll();
  });
  ...
  // ========== WITHDRAW METHOD SWITCHING (instrumented) ==========
  ...
  function setWithdrawMethod(method) {
    ...
    debugLog(`setWithdrawMethod: ${activeWithdrawMethod} → ${method}`);
  ...
  function updateMethodCardsVisibility() {
    debugLog(`updateMethodCardsVisibility for currency ${currentCurrency}`);
    const allCards = {
    ...
    if (allCards.bank) {
      ...
      debugLog(`Bank card display: ${currentCurrency === 'USDT' ? 'none' : 'block'}`);
    }
    // Auto‑switch method if current method is invalid for the currency
    if (currentCurrency === 'USDT' && activeWithdrawMethod === 'bank') {
      debugLog('Auto-switching method to usdt_trc20');
      setWithdrawMethod('usdt_trc20');
    }
    if (currentCurrency !== 'USDT' && activeWithdrawMethod.startsWith('usdt')) {
      debugLog('Auto-switching method to bank');
      setWithdrawMethod('bank');
    }
  }
  ...
  function renderWithdrawForm(method) {
    ...
    debugLog(`renderWithdrawForm called with method=${method}`);
    let html = '';
    ...
    if (currentCurrency === 'USDT') {
        if (method === 'bank' || method === 'ewallet') {
            ...
            activeWithdrawMethod = method;
            document.querySelectorAll('.method-card').forEach(card => card.classList.toggle('active', card.dataset.method === method));
            debugLog('Forced method to usdt_trc20 for USDT');
        }
    }
    ...
    debugLog('Withdrawal form rendered');
  }
  ...
  // View Switching (instrumented)
  function setView(v) {
    debugLog(`setView called with view=${v}`);
    ...
    if (v === 'withdraw') {
      debugLog('Entering Withdraw view, updating hero, method cards, form');
      forceUpdateWithdrawHero();
      updateMethodCardsVisibility();
      renderWithdrawForm(activeWithdrawMethod);
    }
    if (v === 'wallet') {
      debugLog('Entering Wallet view, rendering wallet accounts');
      renderWalletAccounts();
      setTimeout(() => renderWalletAccounts(), 10); // safety net
    }
  ...
  // ========== ULTRA-RELIABLE INITIALIZATION (instrumented) ==========
  function tryInitialize() {
    if (window.__appInitialized) return true;
    debugLog('tryInitialize: checking stored user...');
    const stored = getStoredUser();
    if (!stored) {
      debugLog('No stored user found, will retry');
      return false;
    }
    ...
    debugLog(`User loaded: ${currentUser.username}, homeBalance=${base.homeBalance}`);

    if (currentCurrency === 'USDT') {
      debugLog('Initializing method: usdt_trc20');
      setWithdrawMethod('usdt_trc20');
      updateMethodCardsVisibility();
    } else {
      debugLog('Initializing method: bank');
      setWithdrawMethod('bank');
    }
    ...
    setTimeout(() => {
      ...
      debugLog('Initialization complete (delayed tasks)');
    }, 10);
  ...
  (function pollInit() {
    if (tryInitialize()) {
      debugLog('Initialization successful, setting view to home');
      setView('home');
      return;
    }
    if (++initAttempts > 50) {
      debugLog('Initialization failed after 50 attempts', 'error');
      return;
    }
    setTimeout(pollInit, 100);
  })();

  debugLog('main.js loaded, polling for user');
})();
