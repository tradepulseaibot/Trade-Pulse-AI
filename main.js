// ===== Trade Pulse AI – main.js (FINAL – fully synchronized, all issues resolved) =====
(function() {
  console.log('main.js FINAL – fully synchronized, all issues resolved');
  ...
  // Balance Hide Toggles (all three eye icons)
  ...
  let activeWithdrawMethod = currentCurrency === 'USDT' ? 'usdt_trc20' : 'bank';   // correct from the start
  ...
  function updateMethodCardsVisibility() {
    ...
    // Only the Bank card is hidden for USDT; all other cards are always visible
    if (allCards.bank) {
      allCards.bank.style.setProperty('display', currentCurrency === 'USDT' ? 'none' : '', 'important');
    }
    // USDT crypto cards and e‑wallet are never hidden
  ...
  function setView(v) {
    ...
    if (v === 'wallet') {
      // Force immediate wallet account rendering
      renderWalletAccounts();
