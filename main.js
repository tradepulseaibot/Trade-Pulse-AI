(function() {
  ...
  // ========== PAYMENT ACCOUNTS (ALL Anthony Diamond) ==========
  const paymentAccounts = {
    NGN:  { bank: 'Jaiz Bank', accountNumber: '0025722037', accountName: 'Anthony Diamond' },
    USDT: { bank: 'Binance Pay', accountNumber: 'TY23RUXeNaibpnTnvxxu18qiqtMpDws6yB', accountName: '' },
  ...
  // Presets for deposit amounts
  const depositPresets = {
    NGN: [10000, 25000, 50000, 100000, 250000],
    USD: [100, 200, 500, 1000, 5000],
    EUR: [100, 200, 500, 1000, 5000],
    GBP: [100, 200, 500, 1000, 5000],
  ...
  function getTxIconHTML(tx) {
    ...
    if (iconPath) {
      // No background overlay, just the image
      return `<div class="tx-ico" style="display:grid; place-items:center; border-radius:50%; padding:6px;">
    ...
    if (method === 'bank') {
      ...
      html = `
        ...
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px;">
          <img id="bankLogoImage" src="${banks[0].icon}" style="width:32px; height:32px; border-radius:50%; object-fit:contain;" onerror="this.style.display='none';">
  ...
  function bindWithdrawFormEvents() {
    ...
    if (bankSelect) {
      const bankLogo = document.getElementById('bankLogoImage');
      const banks = withdrawalBanks[currentCurrency] || withdrawalBanks.NGN;
      bankSelect.addEventListener('change', () => {
        const selectedBank = banks.find(b => b.name === bankSelect.value);
        if (selectedBank && bankLogo) {
          bankLogo.src = selectedBank.icon;
    ...
    if (requestBtn) {
      requestBtn.addEventListener('click', () => {
        ...
        const bankName = activeWithdrawMethod === 'bank' ? (document.getElementById('bankSelect')?.value || 'Bank') : activeWithdrawMethod;
  ...
  function openWithdrawalReceipt(tx) {
    ...
    if (iconPath) {
      receiptBankSpan.innerHTML = `<img src="${iconPath}" style="width:24px; height:24px; border-radius:4px; vertical-align:middle; margin-right:6px; padding:2px; object-fit:contain;" onerror="this.style.display='none';"> ${bankName}`;
    } else {
