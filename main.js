// ===== Trade Pulse AI – main.js (FINAL – all features, all fixes) =====
(function() {
  ...
  // ========== TRADING REGULATIONS MODAL ==========
  function showTermsModal() {
    if (localStorage.getItem('acceptedTerms')) return;
  ...
  function showOnboardingStep(index, retryCount = 0) {
    ...
    clearTimeout(window.__onboardingScrollTimer);
    window.__onboardingScrollTimer = setTimeout(() => {
      ...
      arrow.style.left = left + tooltipRect.width / 2 - 12 + 'px';
      arrow.style.top = (step.position === 'bottom' ? top - 10 : top + tooltipRect.height - 10) + 'px';
  ...
  function nextOnboardingStep() {
    ...
    if (prevTarget) { prevTarget.style.boxShadow = ''; prevTarget.style.zIndex = ''; }
  ...
  function tryInitialize() {
    ...
    // Show onboarding only for users who just signed up (session flag)
    if (sessionStorage.getItem('justSignedUp') && !localStorage.getItem('onboardingComplete')) {
      setTimeout(startOnboarding, 5000);
    }

    // Show welcome notifications once per browser (new + old users who haven't seen them)
    if (!localStorage.getItem('hasSeenWelcomeNotifs')) {
      window.addWelcomeNotifications();
      localStorage.setItem('hasSeenWelcomeNotifs', 'true');
    }
    // Clean up sign-up flag after it's been used
    ...
    // Show trading regulations modal after 4 seconds if not accepted
    setTimeout(showTermsModal, 4000);
