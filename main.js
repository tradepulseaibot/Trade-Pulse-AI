(function() {
  ...
  let currentInvestMin = 0;   // actual minimum in the current currency
  ...
  const whiteBackgroundExceptions = [
    ...
    // Nigerian banks (non‑transparent)
  ...
  function renderWalletAccounts() {
    ...
    container.querySelectorAll('button[id^="walletDepositBtn-"]').forEach(btn => {
      btn.addEventListener('click', () => openDepositModal());
    });
    container.querySelectorAll('button[id^="walletWithdrawBtn-"]').forEach(btn => {
      btn.addEventListener('click', () => setView('withdraw'));
    });
  }
  ...
  function renderWithdrawForm(method) {
    ...
    if (method === 'bank') {
      ...
      // Extra fields for specific currencies
      let extraFields = '';
  ...
  function generateWithdrawPresets() {
    ...
    return presets.map(amount => {
      if (amount === 'max') {
        return `<button type="button" class="preset preset-max" id="maxBtn">Max</button>`;
      }
      let label;
      if (currentCurrency === 'NGN') {
        ...
        label = `${symbols[currentCurrency]}${amount.toLocaleString()}`;
      }
    ...
    if (presetsContainer) {
      ...
      document.querySelectorAll('#depositModal .preset').forEach(b => {
        b.addEventListener('click', () => {
          $('depositAmountInput').value = b.dataset.amount;
        });
      });
    }
  ...
  $('confirmDepositBtn')?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      // Update payment modal currency label
  ...
  function openInvestModal(planName, minNgn, planKey) {
    ...
    currentInvestMin = minInCurrency;                        // store for validation
  ...
  $('finalConfirmWithdrawBtn')?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      ['withdrawRouting','withdrawSortCode','withdrawIban','withdrawBic'].forEach(id => {
        const el = $(id);
        if (el) el.value = '';
      });
    }, 2500);
  ...
  if (communityInputField) {
    communityInputField.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') { e.preventDefault(); setCommunityPlanPrompt(); }
    });
  }
