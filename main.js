// ===== Trade Pulse AI – main.js (v2.1 – MutationObserver + instant init) =====
(function() {
  console.log('main.js v2.1 – MutationObserver init active');
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    // If withdraw view is active, re‑render its form instantly
  ...
  // ========== INSTANT INITIALIZATION (MutationObserver) ==========
  ...
  // Watch for the main app becoming visible (login screen hides)
  const mainApp = document.getElementById('mainApp');
  if (mainApp) {
    const observer = new MutationObserver((mutations) => {
      mutations.forEach(mutation => {
        if (mutation.attributeName === 'style' && mainApp.style.display === 'block') {
          initializeApp();
        }
      });
    });
    ...
    // Also check immediately in case it's already visible (edge case)
    if (mainApp.style.display === 'block') initializeApp();
  }

  // Fallback polling (in case MutationObserver misses it)
  let fallbackAttempts = 0;
  const fallbackInterval = setInterval(() => {
    if (appInitialized || fallbackAttempts++ > 60) {
