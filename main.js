(function() {
  ...
  // Payment not detected modal (unchanged)
  ...
  // Deposit-first guard modal (unchanged)
  ...
  let base = {
    ...
    txFee: FEE,
    activePlans: 0,
    planMin1: 7500,   // changed to 7500 NGN (≈5 USDT)
    planMax1: 49999,
    planMin2: 50000,
    planMax2: 199999,
    planMin3: 200000,
  ...
  // ========== COMPREHENSIVE BANK ICON MAP (unchanged) ==========
  const bankIconMap = {
  ...
  function renderWalletAccounts() {
    ...
    if (currentCurrency === 'USDT') {
            ...
            <div class="wallet-account-left">
              ...
              <div class="wallet-account-info">
                <div class="wallet-account-top">
                  <span class="wallet-account-name">USDT (TRC20)</span>
                  <span class="primary-tag">Primary</span>
                </div>
                <div class="wallet-account-value">
                  ${balanceHidden ? '****' : trc20Balance.toFixed(2) + ' USDT'}
                </div>
        ...
        <div class="wallet-account">
          <div class="wallet-account-top-row">
            <div class="wallet-account-left">
              ...
              <div class="wallet-account-info">
                <div class="wallet-account-top">
                  <span class="wallet-account-name">USDT (BEP20)</span>
                </div>
                <div class="wallet-account-value">
                  ${balanceHidden ? '****' : bep20Balance.toFixed(2) + ' USDT'}
                </div>
          ...
          <div class="wallet-account-actions">
            <button class="small-action deposit-amber">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/></svg>Deposit
            </button>
            <button class="small-action btn-disabled" disabled>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21V9"/><path d="m7 14 5-5 5 5"/></svg>Withdraw
      ...
      const secondary = currentCurrency === 'NGN'
        ? `≈ $ ${(base.homeBalance * rates.USD).toFixed(2)}`
        : `≈ ${fmt(base.homeBalance)}`;
        ...
        <div class="wallet-account">
          <div class="wallet-account-top-row">
            <div class="wallet-account-left">
              ...
              <div class="wallet-account-info">
                <div class="wallet-account-top">
                  <span class="wallet-account-name">${name}</span>
                  <span class="primary-tag">Primary</span>
                </div>
                <div class="wallet-account-value">
                  ${balanceHidden ? '****' : balanceInCurrency.toFixed(2) + (currentCurrency === 'USDT' ? ' USDT' : '')}
                </div>
          ...
          <div class="wallet-account-actions">
            <button class="small-action deposit-${colorClass}">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/></svg>Deposit
            </button>
            <button class="small-action btn-disabled" disabled>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21V9"/><path d="m7 14 5-5 5 5"/></svg>Withdraw
    ...
    // Re‑attach deposit button listeners
    container.querySelectorAll('.small-action:not(.btn-disabled)').forEach(btn => {
      btn.addEventListener('click', openDepositModal);
    });
  }

  // ========== UPDATE ALL (now calls renderWalletAccounts) ==========
  function updateAll() {
    if (balanceHidden) {
  ...
  // ----- Balance Hide Toggles (unchanged) -----
  ...
  // ----- Overview Period Chip (unchanged) -----
  ...
  // ----- Withdraw Method Switching (unchanged) -----
  ...
  function renderWithdrawForm(method) {
    ...
    if (method === 'bank') {
      ...
      // Currency‑specific extra fields
      let extraFields = '';
      if (currentCurrency === 'USD') {
        extraFields = `
          <div class="field-label">Routing Number</div>
          <div class="amount-input"><input type="text" id="withdrawRoutingInput" placeholder="9-digit routing number" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
        `;
      } else if (currentCurrency === 'EUR') {
        extraFields = `
          <div class="field-label">IBAN</div>
          <div class="amount-input"><input type="text" id="withdrawIbanInput" placeholder="International Bank Account Number" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
          <div class="field-label">BIC / SWIFT</div>
          <div class="amount-input"><input type="text" id="withdrawBicInput" placeholder="Bank Identifier Code" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
        `;
      } else if (currentCurrency === 'GBP') {
        extraFields = `
          <div class="field-label">Sort Code</div>
          <div class="amount-input"><input type="text" id="withdrawSortCodeInput" placeholder="6-digit sort code (e.g. 20-45-67)" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
          <div class="field-label">IBAN</div>
          <div class="amount-input"><input type="text" id="withdrawIbanInput" placeholder="IBAN" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
        `;
      }
  ...
  function bindWithdrawFormEvents() {
    ...
    if (selectEl) {
      ...
      const sourceList = (activeWithdrawMethod.startsWith('usdt'))
    ...
    if (requestBtn) {
      requestBtn.addEventListener('click', () => {
        ...
        const bankName = activeWithdrawMethod === 'bank'
  ...
  // ========== DEPOSIT FLOW (currency sync fix) ==========
  ...
  $('confirmDepositBtn')?.addEventListener('click', () => {
    ...
    // Force sync currentCurrency from the UI button before showing payment modal
    const btnText = $('currencyBtn').textContent.trim().split(' ')[0];
    if (currentCurrency !== btnText) {
      currentCurrency = btnText;
      updateAll(); // refresh all values to match new currency
    }
  ...
  // ========== SHARED PAYMENT MODAL (unchanged) ==========
  ...
  // ========== INVESTMENT FLOW (unchanged except planMin1 fix) ==========
  ...
  $('confirmInvestBtn')?.addEventListener('click', () => {
    ...
    if (amount < minNgn) {
      if (investErr) { investErr.textContent = `Minimum investment is ${symbols[currentCurrency]}${minNgn.toFixed(2)}`; investErr.style.display = 'block'; setTimeout(() => investErr.style.display = 'none', 3000); }
      return;
    }
  ...
  // ========== Community Plan Prompt (unchanged) ==========
  ...
  // ========== VIP Upgrade Scroll Fix (unchanged) ==========
  ...
  // ========== Wallet Action Buttons (unchanged) ==========
  ...
  // ========== Modal overlay click to close (unchanged) ==========
  ...
  // ========== WITHDRAWAL RECEIPT WITH ICON (unchanged) ==========
