// ===== Trade Pulse AI – main.js (FINAL – working tabs, all issues resolved) =====
(function() {
  console.log('main.js FINAL – working tabs, all issues resolved');
  ...
  // ========== FORCE WITHDRAWAL HERO SYMBOL (respects balanceHidden) ==========
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    updateAll();
    forceUpdateWithdrawHero();   // ensure withdrawal hero symbol updates immediately
  ...
  function updateMethodCardsVisibility() {
    ...
    // Bank – only for non‑USDT currencies
    if (allCards.bank) {
      allCards.bank.style.setProperty('display', currentCurrency === 'USDT' ? 'none' : '', 'important');
    }

    // USDT crypto cards – only for USDT
    if (allCards.usdt_trc20) {
      allCards.usdt_trc20.style.setProperty('display', currentCurrency === 'USDT' ? '' : 'none', 'important');
    }
    if (allCards.usdt_bep20) {
      allCards.usdt_bep20.style.setProperty('display', currentCurrency === 'USDT' ? '' : 'none', 'important');
    }

    // E‑wallet always visible (already fine)
  ...
  // ========== IMMEDIATELY HIDE BANK CARD FOR USDT DEFAULT ==========
  updateMethodCardsVisibility();
