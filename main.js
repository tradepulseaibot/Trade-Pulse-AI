// ===== Trade Pulse AI – main.js (FINAL – all fixes, no undefined) =====
(function() {
  ...
  $('notificationPreviewModal')?.addEventListener('click', (e) => { if (e.target === $('notificationPreviewModal')) closeModal('notificationPreviewModal'); });

  // Forced demo modals
  function showKycUnverifiedModal() { /* ... same as before ... */ }
  function showKycProcessingModal() { /* ... same as before ... */ }
  function showKycRejectedDetailModal() { /* ... same as before ... */ }
  ...
  function blockIfForcedDemo() {
    ...
    const forcedKyc = getForcedDemoKyc();
    const status = forcedKyc?.status || 'unverified';
    if (status === 'processing') { showKycProcessingModal(); }
    else if (status === 'rejected') { showKycRejectedDetailModal(); }
    else { showKycUnverifiedModal(); }
    return true;
  }

  // Admin toggle UI
  function addAdminToggleUI() { /* ... same as before ... */ }

  function openAdminPanel() { /* ... same as before ... */ }

  // 2-day auto-reject timer
  setInterval(() => { /* ... same as before ... */ }, 60000);

  // Forced demo growth stop after 2 days
  let demoKycModalShown = false;
  function checkAndStopDemoGrowth() { /* ... same as before ... */ }
  ...
  let bannerInterval;
  function showBanner(index) { /* ... same as before ... */ }
  ...
  if (bannerEl) { /* ... */ }
  function toggleBanner(active) { if (active) { showBanner(0); startBannerAutoPlay(); } else stopBannerAutoPlay(); }

  // Withdrawal Receipt
  function openWithdrawalReceipt(tx) { /* ... same as before ... */ }
  function handleWithdrawalRowClick(e) { /* ... same as before ... */ }
  ...
  function showTermsModal() { /* ... same as before ... */ }
  window.showTermsModal = showTermsModal;
  $('acceptTermsCheckbox')?.addEventListener('change', function() { $('acceptTermsBtn').disabled = !this.checked; });
  ...
  function initializeApp() {
    ...
    if (isForcedDemoActive()) { /* ... */ }
    else { base.homeBalance = stored.homeBalance || 0; base.totalInvested = stored.totalInvested || 0; base.totalWithdrawn = stored.totalWithdrawn || 0; base.activePlans = stored.activePlans || 0; }

    if (currentCurrency === 'USDT') { setWithdrawMethod('usdt_trc20'); updateMethodCardsVisibility(); }
    else { setWithdrawMethod('bank'); }
    ...
    document.addEventListener('click', (e) => { if (e.target.closest('button, .btn, .nav-item, .action, .drawer-menu-item, .invest-now-btn, .small-action')) playClickSound(); });

    const uid = stored.userId || stored.username;
    if (sessionStorage.getItem('justSignedUp')) {
      if (!localStorage.getItem('hasSeenWelcomeNotifs_' + uid)) { window.addWelcomeNotifications(); localStorage.setItem('hasSeenWelcomeNotifs_' + uid, 'true'); }
      if (!localStorage.getItem('acceptedTerms_' + uid)) showTermsModal();
    ...
    setTimeout(() => { renderWalletAccounts(); forceUpdateWithdrawHero(); if (activeWithdrawMethod) renderWithdrawForm(activeWithdrawMethod); }, 10);
  ...
  function unlockAudio() {
    ...
    sounds.forEach(id => { const audio = document.getElementById(id); if (audio) { audio.volume = 0; audio.play().then(() => { audio.pause(); audio.currentTime = 0; audio.volume = 1; }).catch(()=>{}); } });
    audioUnlocked = true;
  }
