// ===== Trade Pulse AI – main.js (FINAL PRODUCTION VERSION with drawer KYC badge) =====
(function() {
  ...
  // ----- UPDATE DRAWER USER INFO (with KYC badge clickable) -----
  function updateDrawerUserInfo() {
    ...
    if (drawerMembership) {
      // Keep existing text node for membership, then handle KYC badge
    ...
    if (kycBadge) {
      ...
      // Make it clickable – call profile.js's openKycModal if exists
      kycBadge.onclick = function(e) {
        e.stopPropagation();
        if (typeof window.openKycModal === 'function') {
          window.openKycModal();
        }
      };
      kycBadge.style.cursor = 'pointer';
    }
