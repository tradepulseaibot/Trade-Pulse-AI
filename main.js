(function() {
  ...
  function updateAll() {
    ...
    // Always update plan amounts
    ['planMin1','planMax1','planMin2','planMax2','planMin3','planMax3','planMin4'].forEach(id => {
      const el = $(id);
      if (el && base[id] !== undefined) el.textContent = fmt(base[id]);
    });
    ...
    if (balanceHidden) {
      ...
      updateEyeIcons();
      try { renderWalletAccounts(); } catch(e) {}
      return;
    }
  ...
  function renderWithdrawForm(method) {
    ...
    // Extra safety: ensure bank card hidden when USDT
    if (currentCurrency === 'USDT') {
      const bankCard = document.querySelector('.method-card[data-method="bank"]');
  ...
  function setView(v) {
    ...
    updateAll();
    if (v === 'wallet') try { renderWalletAccounts(); } catch(e) {}
    toggleBanner(v === 'home');
