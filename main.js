...
(function() {
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    updateMethodCardsVisibility();  // Fix #2
    updateAll();                     // updates everything including wallet accounts
    ...
    // Force wallet accounts render (not dependent on eye toggle)
    renderWalletAccounts();
  });
  ...
  function setView(v) {
    ...
    // Immediately render wallet accounts when entering wallet view
    if (v === 'wallet') {
      setTimeout(() => renderWalletAccounts(), 10);
    }
    // Ensure bank card visibility and form update when entering withdraw view
    if (v === 'withdraw') {
  ...
  // Extra safety for wallet accounts on load
  setTimeout(() => {
