// ===== Trade Pulse AI – main.js (COMPLETE DEBUG VERSION – on‑screen logs) =====
(function() {
  console.log('main.js loaded – DEBUG version');
  ...
  (function createDebugOverlay() {
    ...
    div.id = 'debugOverlay';
    div.style.cssText =
      'position:fixed;bottom:0;left:0;right:0;max-height:30vh;overflow-y:auto;' +
      'background:rgba(0,0,0,0.85);color:#0f0;font:12px monospace;padding:8px;z-index:9999;';
  ...
  // ========== FIX #3: DYNAMIC WALLET ACCOUNTS ==========
  function renderWalletAccounts() {
    debugLog(`renderWalletAccounts() called. currency=${currentCurrency}`);
    let container = document.getElementById('walletAccountsContainer');
    if (!container) {
      debugLog('walletAccountsContainer missing – creating dynamically');
      ...
      else { debugLog('ERROR: walletView not found'); return; }
    }
    ...
    debugLog('walletAccountsContainer updated');
  }

  // ========== FIX #1: FORCE WITHDRAWAL HERO SYMBOL ==========
  function forceUpdateWithdrawHero() {
    debugLog(`forceUpdateWithdrawHero() called. currency=${currentCurrency}`);
    ...
    if (withdrawBalanceEl) {
      ...
      debugLog(`withdrawBalance set to ${withdrawBalanceEl.textContent}`);
    } else debugLog('withdrawBalance element NOT FOUND');
    if (withdrawFxEl) withdrawFxEl.textContent = `≈ $ ${(base.homeBalance * rates['USD']).toFixed(2)}`;
  ...
  // ========== UPDATE ALL ==========
  function updateAll() {
    debugLog(`updateAll() running. currency=${currentCurrency} balanceHidden=${balanceHidden}`);
    ...
    ['planMin1','planMax1','planMin2','planMax2','planMin3','planMax3','planMin4'].forEach(id => {
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    debugLog(`CURRENCY SWITCH to ${currentCurrency}`);
    ...
    if (currentCurrency !== 'USDT' && activeWithdrawMethod.startsWith('usdt')) {
      debugLog('Resetting method to bank (not USDT)');
    ...
    if (document.getElementById('withdrawView').classList.contains('active')) {
      debugLog('withdrawView is active → re‑rendering form');
  ...
  function setWithdrawMethod(method) {
    debugLog(`setWithdrawMethod: current=${activeWithdrawMethod} new=${method}`);
    if (!withdrawFormContainer || method === activeWithdrawMethod) {
      debugLog('No change or container missing, returning');
      return;
    }
  ...
  function updateMethodCardsVisibility() {
    ...
    if (bankCard) {
      if (currentCurrency === 'USDT') {
        bankCard.style.setProperty('display', 'none', 'important');
        debugLog('Bank card hidden');
      } else {
        bankCard.style.setProperty('display', '', 'important');
        debugLog('Bank card shown');
      }
    } else {
      debugLog('Bank card element NOT FOUND');
    }
    ...
    if (currentCurrency !== 'USDT' && activeWithdrawMethod.startsWith('usdt')) {
      debugLog('Switching method to bank (since currency is not USDT)');
      setWithdrawMethod('bank');
    }
  }

  function renderWithdrawForm(method) {
    debugLog(`renderWithdrawForm() method=${method} currency=${currentCurrency}`);
