...
(function() {
  ...
  // Referral modal "How does it work?" toggle – global delegation
  function initReferHowToggle() {
    document.addEventListener('click', (e) => {
      const title = e.target.closest('#referHowTitle');
      if (!title) return;
  ...
  const onboardingSteps = [
    { target: '#homeBalance', title: 'Your Balance', text: 'This shows your total account balance. It updates as you trade and earn profits.', position: 'bottom' },
    { target: '.nav-item[data-view="invest"]', title: 'Invest', text: 'Explore our investment plans here. Choose a plan that fits your goals and start earning daily profits.', position: 'top' },
    { target: '.nav-item[data-view="withdraw"]', title: 'Withdraw', text: 'Request withdrawals from your available balance. Funds are processed quickly.', position: 'top' },
    { target: '.nav-item[data-view="community"]', title: 'Community', text: 'Join the community chat, share wins, and see live withdrawal proofs from other members.', position: 'top' },
    { target: '#menuBtn', title: 'Menu', text: 'Open the drawer to access your profile, security settings, and more options.', position: 'top' }
  ];
  ...
  function showOnboardingStep(index, retryCount = 0) {
    ...
    if (!targetEl && retryCount < 5) {
      setTimeout(() => showOnboardingStep(index, retryCount + 1), 500);
      return;
    }
    if (!targetEl) {
      nextOnboardingStep();
    ...
    if (textEl) {
      textEl.innerHTML = `<h3 style="margin-top:0">${step.title}</h3><p>${step.text}</p>`;
    }
  ...
  function tryInitialize() {
    ...
    // Start onboarding tour after 5 seconds if not completed
    if (!localStorage.getItem('onboardingComplete')) {
      setTimeout(startOnboarding, 5000);
    }

    // Welcome notifications on sign‑up
