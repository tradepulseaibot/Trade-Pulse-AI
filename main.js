(function() {
  ...
  // ========== WITHDRAWAL BANKS PER CURRENCY (USDT now serves as wallet list) ==========
  ...
  function getTxIconHTML(tx) {
    ...
    if (iconPath) {
      return `<div class="tx-ico" style="background:#ffffff; display:grid; place-items:center; border-radius:50%; padding:6px;">
    ...
    if (method === 'bank') {
      ...
      html = `
        ...
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px;">
          <img id="bankLogoImage" src="${banks[0].icon}" style="width:32px; height:32px; border-radius:50%; object-fit:contain; background:#fff;" onerror="this.style.display='none';">
      ...
      const wallets = withdrawalBanks.USDT;   // use the USDT array as wallet list
      const options = wallets.map(w => `<option value="${w.name}">${w.name}</option>`).join('');
      html = `
        <div class="field-label">Select Wallet / Exchange</div>
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px;">
          <img id="walletLogoImage" src="${wallets[0].icon}" style="width:32px; height:32px; border-radius:50%; object-fit:contain; background:#fff;" onerror="this.style.display='none';">
  ...
  function openWithdrawalReceipt(tx) {
    ...
    if (iconPath) {
      receiptBankSpan.innerHTML = `<img src="${iconPath}" style="width:24px; height:24px; border-radius:4px; vertical-align:middle; margin-right:6px; background:#ffffff; padding:2px; object-fit:contain;" onerror="this.style.display='none';"> ${bankName}`;
    } else {
