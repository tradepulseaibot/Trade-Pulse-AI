(function() {
  ...
  const paymentAccounts = {
    ...
    USDT: { bank: 'USDT (TRC20)', accountNumber: 'TY23RUXeNaibpnTnvxxu18qiqtMpDws6yB', accountName: 'Anthony Diamond' },
  ...
  // Presets for withdrawal amounts (full numbers)
  ...
  function getTxIconHTML(tx) {
    ...
    if (iconPath) {
      return `<div class="tx-ico" style="background:#ffffff; overflow:hidden; display:grid; place-items:center; border-radius:50%; padding:6px;">
  ...
  // ----- Currency Switcher (closes deposit/payment modals, updates withdrawal form) -----
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    // Refresh the withdrawal form to update bank list and logo
  ...
  function bindWithdrawFormEvents() {
    ...
    // Update bank logo on selection change
  ...
  function updateDepositModalCurrency() {
    ...
    if (presetsContainer) {
      presetsContainer.innerHTML = presets.map(amount => {
        const label = currentCurrency === 'USDT' ? `${amount} USDT` : `${symbols[currentCurrency]}${amount.toLocaleString()}`;
        return `<button type="button" class="preset" data-amount="${amount}">${label}</button>`;
      }).join('');
      // Reattach event listeners for the newly created preset buttons
  ...
  function openDepositModal() {
    updateDepositModalCurrency(); // always refresh presets & currency before showing
  ...
  $('confirmDepositBtn')?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      hideProcessing();
      const amountInNGN = amount * (toNGN[currentCurrency] || 1);
      $('paymentAmount').textContent = fmt(amountInNGN);
      ...
      accNumEl.style.wordBreak = 'break-all';  // Prevent long address from cutting off
      let extraInfo = '';
  ...
  // ========== INVESTMENT FLOW (always NGN account) ==========
  ...
  $('confirmInvestBtn')?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      // Force payment modal to show NGN account for investment
      const account = paymentAccounts.NGN;
      document.querySelector('#paymentModal .payment-account-name').textContent = account.bank;
      const accNumEl = document.querySelector('#paymentModal .payment-account-number');
      accNumEl.style.wordBreak = 'break-all';
      accNumEl.textContent = account.accountNumber;
      document.querySelector('#paymentModal .payment-account-bank').textContent = account.accountName;
