// ===== Trade Pulse AI – main.js (FINAL – all issues resolved) =====
(function() {
  ...
  function renderWalletAccounts() {
    ...
    if (!container) { console.warn('Wallet container not found'); return; }
  ...
  // ========== UPDATE ALL (Fix #1 – withdraw hero symbol) ==========
  function updateAll() {
    ...
    // Fix #1: withdrawal hero always uses current symbol
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    // Fix #4: Re‑render withdrawal form instantly if withdraw view is active
    if ($('withdrawView').classList.contains('active')) {
      renderWithdrawForm(activeWithdrawMethod);
    }
    // Fix #2: Bank card hidden when USDT
    ...
    if ($('walletView').classList.contains('active')) {
      setTimeout(() => renderWalletAccounts(), 50);
    } else {
      renderWalletAccounts();
    }
  });
  ...
  // ========== WITHDRAW METHOD SWITCHING (Fix #2) ==========
  ...
  function updateMethodCardsVisibility() {
    ...
    if (bankCard) {
      if (currentCurrency === 'USDT') {
        bankCard.style.setProperty('display', 'none', 'important'); // Fix #2
      } else {
        bankCard.style.setProperty('display', '', 'important');
      }
    }
  ...
  function renderWithdrawForm(method) {
    ...
    // If USDT is selected, never show bank fields
    if (currentCurrency === 'USDT') {
  ...
  function setView(v) {
    ...
    if (v === 'wallet') {
      try { renderWalletAccounts(); } catch(e) { console.error(e); }
      setTimeout(() => { try { renderWalletAccounts(); } catch(e) { console.error(e); } }, 200);
    }
