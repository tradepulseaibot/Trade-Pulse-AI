// ===== Trade Pulse AI – main.js (FINAL – all issues permanently resolved) =====
(function() {
  console.log('main.js loaded – FINAL version');
  ...
  let checkCount = 0;   // for polling initialisation
  ...
  // ========== FORCE WITHDRAWAL HERO SYMBOL (Fix #1) ==========
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    // Fix #2 + #4: update method cards and form before anything else
    updateMethodCardsVisibility();

    // Force method switching according to new currency
    if (currentCurrency === 'USDT') {
      // Auto‑switch to USDT TRC20 if we aren't already on a crypto method
      if (!activeWithdrawMethod.startsWith('usdt')) {
        setWithdrawMethod('usdt_trc20');
      }
    } else {
      // Switch back to bank if we were on a USDT method
      if (activeWithdrawMethod.startsWith('usdt')) {
    ...
    updateAll();
    forceUpdateWithdrawHero();   // ensure withdrawal hero updates immediately
    updateDepositModalCurrency();

    // Force wallet accounts update (even if not currently on wallet view)
    renderWalletAccounts();

    // Re‑render withdrawal form instantly if on that view
  ...
  // ----- Polling initialisation (handles login that happens after main.js runs) -----
  (function initAppFromStoredUser() {
    ...
    if (stored && !currentUser) {
      ...
      updateCardDisplay();
      updateMethodCardsVisibility();
      // Final safety net after everything is loaded
      setTimeout(() => {
        renderWalletAccounts();
        forceUpdateWithdrawHero();
      }, 200);
    } else {
      // User not yet logged in – try again in 500ms (up to 60 attempts)
      if (checkCount++ < 60) setTimeout(initAppFromStoredUser, 500);
    }
  })();
