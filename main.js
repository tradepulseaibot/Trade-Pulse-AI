// Trade Pulse AI – main.js (final version – all issues resolved)
(function() {
  ...
  function renderWalletAccounts() {
    ...
    if (!container) { console.warn('Wallet accounts container not found'); return; }
  ...
  $('currencyDropdown').addEventListener('click', e => {
    ...
    if ($('walletView').classList.contains('active')) {
      setTimeout(() => renderWalletAccounts(), 30);
    }
  });
  ...
  function setView(v) {
    ...
    updateAll();
    if (v === 'wallet') setTimeout(() => renderWalletAccounts(), 50);
    toggleBanner(v === 'home');
