(function() {
  ...
  // ========== WITHDRAWAL BANKS PER CURRENCY (comprehensive) ==========
  ...
  // Presets for deposit amounts – NGN starts at 10k, others at 10
  ...
  // Presets for withdrawal amounts
  ...
  // Conversion factors from selected currency to NGN
  const toNGN = {
    NGN: 1,
    USD: 1 / 0.00067,
    EUR: 1 / 0.00061,
  ...
  const bankIconMap = {
    ...
    'Chase': 'assets/banks/chase.png',
    'Wells Fargo': 'assets/banks/wells_fargo.png',
    'Citibank': 'assets/banks/citibank.png',
    'Deutsche Bank': 'assets/banks/deutsche.png',
    'BNP Paribas': 'assets/banks/bnp_paribas.png',
    'ING': 'assets/banks/ing.png',
    'Santander': 'assets/banks/santander.png',
    'Barclays': 'assets/banks/barclays.png',
    'HSBC UK': 'assets/banks/hsbc_uk.png',
    'Lloyds': 'assets/banks/lloyds.png',
    'NatWest': 'assets/banks/natwest.png',
    'Binance Pay': 'assets/banks/binance.png',
    'Bybit': 'assets/banks/bybit.png',
    'Coinbase': 'assets/banks/coinbase.png',
    'OKX': 'assets/banks/okx.png',
    'Kraken': 'assets/banks/kraken.png',
    'Trust Wallet': 'assets/banks/trustwallet.png',
    'MetaMask': 'assets/banks/metamask.png',
    'KuCoin': 'assets/banks/kucoin.png',
    'Huobi': 'assets/banks/huobi.png',
    'PayPal': 'assets/banks/paypal.png',
    'Cash App': 'assets/banks/cashapp.png',
    'Venmo': 'assets/banks/venmo.png',
    'Zelle': 'assets/banks/zelle.png',
    'USAA': 'assets/banks/usaa.png',
    'Revolut': 'assets/banks/revolut.png',
    'N26': 'assets/banks/n26.png',
    'Bunq': 'assets/banks/bunq.png',
    'Commerzbank': 'assets/banks/commerzbank.png',
    'Vivid': 'assets/banks/vivid.png',
    'Monzo': 'assets/banks/monzo.png',
    'Starling': 'assets/banks/starling.png',
    'Revolut UK': 'assets/banks/revolut_uk.png',
    'Metro Bank': 'assets/banks/metro.png',
    'Halifax': 'assets/banks/halifax.png',
    'USDT': 'assets/banks/usdt.png',
    'USDT TRC20 Network': 'assets/banks/usdt.png',
  ...
  // Non‑transparent logos (do NOT need a white background)
  const whiteBackgroundExceptions = [
    'Binance Pay', 'Bybit', 'Cash App', 'Vivid', 'USAA',
    'GTBank', 'Access Bank', 'Opay', 'First Bank', 'UBA',
    'Zenith Bank', 'Palmpay', 'Kuda Bank', 'Wema Bank'
  ];

  function shouldUseWhiteBackground(name) {
    return !whiteBackgroundExceptions.includes(name);
  }

  // FALLBACK SVG WITH SINGLE QUOTES (safe for inline HTML attributes)
  ...
  function renderWalletAccounts() {
    const container = document.querySelector('#walletCardList');
    if (!container) return;

    let accounts = [];
    if (currentCurrency === 'USDT') {
      accounts = [
        { name: 'USDT Wallet (TRC20)', symbol: '₮', value: base.homeBalance * rates.USDT },
        { name: 'USDT Wallet (BEP20)', symbol: '₮', value: base.homeBalance * rates.USDT }
      ];
    } else {
      const symbol = symbols[currentCurrency] || '';
      const value = base.homeBalance * (rates[currentCurrency] || 1);
      accounts = [
        { name: `${currentCurrency} Wallet`, symbol, value }
      ];
    }

    container.innerHTML = accounts.map((acc, idx) => {
      const primaryTag = (idx === 0) ? '<span class="primary-tag">Primary</span>' : '';
      const currencyDisplay = acc.symbol + ' ' + acc.value.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      let coinClass = 'purple';
      if (currentCurrency === 'USDT') coinClass = idx === 0 ? 'green' : 'amber';
      else if (currentCurrency === 'USD') coinClass = 'blue';
      else if (currentCurrency === 'EUR') coinClass = 'purple';
      else if (currentCurrency === 'GBP') coinClass = 'blue';
      else coinClass = 'purple';

      return `
        <div class="wallet-account">
          <div class="wallet-account-top-row">
            <div class="wallet-account-left">
              <div class="wallet-coin ${coinClass}">${acc.symbol}</div>
              <div class="wallet-account-info">
                <div class="wallet-account-top">
                  <span class="wallet-account-name">${acc.name}</span> ${primaryTag}
                </div>
                <div class="wallet-account-value">${currencyDisplay}</div>
                <div class="wallet-account-sub">Available</div>
              </div>
            </div>
          ...
          <div class="wallet-account-actions">
            <button class="small-action deposit-purple" id="walletDepositBtn-${idx}">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/></svg>Deposit
            </button>
            <button class="small-action btn-disabled" id="walletWithdrawBtn-${idx}" disabled>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21V9"/><path d="m7 14 5-5 5 5"/></svg>Withdraw
            </button>
          </div>
        </div>
      `;
    ...
    container.querySelectorAll('button[id^="walletDepositBtn-"]').forEach(btn => btn.addEventListener('click', () => openDepositModal()));
    container.querySelectorAll('button[id^="walletWithdrawBtn-"]').forEach(btn => btn.addEventListener('click', () => setView('withdraw')));
  }
  ...
  function updateMethodCardsVisibility() {
    ...
    if (bankCard) {
      bankCard.style.display = (currentCurrency === 'USDT') ? 'none' : '';
    }
    if (currentCurrency === 'USDT' && activeWithdrawMethod === 'bank') {
  ...
  function renderWithdrawForm(method) {
    ...
    if (method === 'bank') {
      ...
      if (currentCurrency === 'USD') {
        extraFields = `<div class="field-label">Routing Number</div>
          <div class="amount-input"><input type="text" id="withdrawRouting" placeholder="Enter routing number" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>`;
      } else if (currentCurrency === 'GBP') {
        extraFields = `<div class="field-label">Sort Code</div>
          <div class="amount-input"><input type="text" id="withdrawSortCode" placeholder="00-00-00" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
          <div class="field-label">IBAN (optional)</div>
          <div class="amount-input"><input type="text" id="withdrawIban" placeholder="GB29 BARC 204567 47289163" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>`;
      } else if (currentCurrency === 'EUR') {
        extraFields = `<div class="field-label">IBAN</div>
          <div class="amount-input"><input type="text" id="withdrawIban" placeholder="DE12 5007 0010 9876 5432 10" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>
          <div class="field-label">BIC / SWIFT</div>
          <div class="amount-input"><input type="text" id="withdrawBic" placeholder="DEUTDEFFXXX" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div>`;
      }
      ...
      html = `
        ...
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px;">
          ...
          <select id="bankSelect" style="flex:1; height:56px; border-radius:16px; border:1px solid rgba(255,255,255,.08); background:#0b1020; color:#fff; padding:0 16px; font-size:16px; cursor:pointer;">
            ${options}
          </select>
        </div>
      ...
      html = `
        ...
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px;">
          ...
          <select id="walletSelect" style="flex:1; height:56px; border-radius:16px; border:1px solid rgba(255,255,255,.08); background:#0b1020; color:#fff; padding:0 16px; font-size:16px; cursor:pointer;">
            ${options}
          </select>
        </div>
  ...
  function bindWithdrawFormEvents() {
    ...
    if (selectEl) {
      ...
      const sourceList = (activeWithdrawMethod === 'usdt_trc20' || activeWithdrawMethod === 'usdt_bep20')
                         ? withdrawalBanks.USDT
                         : (withdrawalBanks[currentCurrency] || withdrawalBanks.NGN);
      selectEl.addEventListener('change', () => {
        const selected = sourceList.find(i => i.name === selectEl.value);
        if (selected && logoEl) {
          logoEl.src = selected.icon;
          logoEl.style.display = 'block';
          if (shouldUseWhiteBackground(selected.name)) {
    ...
    if (requestBtn) {
      requestBtn.addEventListener('click', () => {
        ...
        const routing = ($('withdrawRouting')?.value || '').trim();
        const sortCode = ($('withdrawSortCode')?.value || '').trim();
        const iban = ($('withdrawIban')?.value || '').trim();
        const bic = ($('withdrawBic')?.value || '').trim();
        ...
        let bankName = activeWithdrawMethod === 'bank'
                       ? (document.getElementById('bankSelect')?.value || 'Bank')
                       : (document.getElementById('walletSelect')?.value || activeWithdrawMethod);

        let bankInfo = bankName;
        if (routing) bankInfo += ` | Routing: ${routing}`;
        if (sortCode) bankInfo += ` | Sort: ${sortCode}`;
        if (iban) bankInfo += ` | IBAN: ${iban}`;
        if (bic) bankInfo += ` | BIC: ${bic}`;

        const fee = activeWithdrawMethod.startsWith('usdt') ? 1 : FEE;
        const receive = Math.max(0, amount - fee);
        $('confirmBank').textContent = `${bankInfo} - ${accountNumber}`;
  ...
  function updateDepositModalCurrency() {
    ...
    if (presetsContainer) {
      presetsContainer.innerHTML = presets.map(amount => {
        const label = `${symbols[currentCurrency]}${amount.toLocaleString()}`;
        return `<button type="button" class="preset" data-amount="${amount}">${label}</button>`;
      }).join('');
      document.querySelectorAll('#depositModal .preset').forEach(b => {
        b.addEventListener('click', () => { $('depositAmountInput').value = b.dataset.amount; });
      });
    }
  ...
  $('confirmDepositBtn')?.addEventListener('click', () => {
    ...
    if (currentCurrency === 'NGN' && amount < 10000) {
      err.textContent = 'Minimum deposit is ₦10,000'; err.style.display = 'block';
      setTimeout(() => err.style.display = 'none', 3000);
      return;
    }
    if (currentCurrency !== 'NGN' && amount < 10) {
      const sym = symbols[currentCurrency] || '';
      err.textContent = `Minimum deposit is ${sym}10`; err.style.display = 'block';
      setTimeout(() => err.style.display = 'none', 3000);
      return;
    }
    closeModal('depositModal');
    ...
    setTimeout(() => {
      ...
      const paymentCurrencyLabel = document.querySelector('#paymentModal .currency');
      if (paymentCurrencyLabel) paymentCurrencyLabel.textContent = currentCurrency;
  ...
  $('confirmPaymentBtn')?.addEventListener('click', () => {
    ...
    setTimeout(() => { hideProcessing(); showPaymentNotDetected(); }, 2000);
  });
  ...
  function openInvestModal(planName, minNgn, planKey) {
    ...
    $('investAmountInput').value = minInCurrency;
    const currencySpan = document.querySelector('#investModal .currency');
    if (currencySpan) currencySpan.textContent = currentCurrency;
  ...
  $('confirmInvestBtn')?.addEventListener('click', () => {
    ...
    if (amount < currentInvestMin) {
      if (investErr) { investErr.textContent = `Minimum investment is ${symbols[currentCurrency]}${currentInvestMin.toFixed(2)}`; investErr.style.display = 'block'; setTimeout(() => investErr.style.display = 'none', 3000); }
      return;
    }
  ...
  $('finalConfirmWithdrawBtn')?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      ['withdrawRouting','withdrawSortCode','withdrawIban','withdrawBic'].forEach(id => { const el = $(id); if (el) el.value = ''; });
    }, 2500);
  });
  ...
  function setCommunityPlanPrompt() {
    ...
    modal.innerHTML = `
      <div class="modal-panel" style="text-align:center;">
        <div class="modal-header"><div class="modal-title">Community Chat</div><button class="modal-close-btn" id="closeCommunityPlanPrompt">✕</button></div>
        <div class="notice-box"><div class="notice-text">Please select an investment plan first to participate in the community chat.</div></div>
        <div style="display:flex; gap:12px; margin-top:20px;">
  ...
  $('upgradeVipBtn')?.addEventListener('click', () => {
    setView('invest');
    setTimeout(() => { const vipCard = $('vipPlanCard'); if (vipCard) vipCard.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 300);
  });
