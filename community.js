// ===== Trade Pulse AI – community.js (FINAL – all fixes, persistent pill, typing guard, new currencies) =====
(function() {
  // ─── Force‑override any CSS that might clip chat cards ───
  ...
  console.log('community.js loaded (final)');
  ...
  const personaData = [
    ...
    // ----- FORMER PROOF PERSONAS (now regular chatters, duplicate removed) -----
    { name: "armando", gender: "men", country: "Mexico", isFallback: false },
    { name: "bibek humagain", gender: "men", country: "India", isFallback: false },
    { name: "bob decker", gender: "men", country: "US", isFallback: false },
    { name: "chris mccabe", gender: "men", country: "US", isFallback: false },
    { name: "daniel bleckie", gender: "men", country: "US", isFallback: false },
    { name: "dennis ikpone", gender: "men", country: "Nigeria", isFallback: false },
    { name: "dora", gender: "women", country: "US", isFallback: false },
    { name: "ed reiter", gender: "men", country: "US", isFallback: false },
    { name: "emmarrio willie", gender: "men", country: "US", isFallback: false },
    { name: "honesto lang", gender: "men", country: "Mexico", isFallback: false },
    { name: "jason mark", gender: "men", country: "US", isFallback: false },
    { name: "josue escobar", gender: "men", country: "Mexico", isFallback: false },
    { name: "madison", gender: "women", country: "US", isFallback: false },
    { name: "maij heev", gender: "men", country: "US", isFallback: false },
    { name: "orville douglas", gender: "men", country: "US", isFallback: false },
    { name: "paul burgess", gender: "men", country: "US", isFallback: false },
    { name: "paul kohler", gender: "men", country: "US", isFallback: false },
    { name: "rich myrie", gender: "men", country: "US", isFallback: false },
    { name: "shonda aires", gender: "women", country: "US", isFallback: false },
    { name: "simon nunn", gender: "men", country: "US", isFallback: false },
    { name: "tanya davenport", gender: "women", country: "US", isFallback: false },
    { name: "terri martis", gender: "women", country: "US", isFallback: false },
    { name: "valeria escamilla", gender: "women", country: "Mexico", isFallback: false },
    { name: "vickie hysell", gender: "women", country: "US", isFallback: false },
    { name: "victoria jeffords", gender: "women", country: "US", isFallback: false },
    { name: "wendy blackston", gender: "women", country: "US", isFallback: false },
    { name: "debbie stroebel", gender: "women", country: "US", isFallback: false },
    { name: "timini davis", gender: "women", country: "US", isFallback: false },
    { name: "benjamin madrigal", gender: "men", country: "Mexico", isFallback: false },
    // 4 Moderators
  ...
  // ========== (Proof card code completely removed) ==========
  ...
  function applyTypos(text, persona) {
    ...
    if (word.length > 3 && Math.random() < 0.4) { const pos = Math.floor(Math.random()*(word.length-1)); const chars=word.split(''); [chars[pos],chars[pos+1]]=[chars[pos+1],chars[pos]]; words[idx]=chars.join(''); }
    return words.join(' ');
  }
  function applySlang(text, persona) {
    if (persona.traits.slang < Math.random()) return text;
    const slangMap = {"going to":"gonna","want to":"wanna","you all":"y'all","I am":"I'm","you are":"you're","cannot":"can't","do not":"don't","does not":"doesn't","let us":"let's","what is":"what's","it is":"it's","that is":"that's"};
  ...
  function getRandomMessage(persona) {
    ...
    if (slangMsg) { let msg=slangMsg; msg=applySlang(msg,persona); msg=applyTypos(msg,persona); if (persona.traits.slang>0.6 && Math.random()<0.3) msg+=' '+['🔥','😂','💰','😎','👍'][Math.floor(Math.random()*5)]; return msg; }
  ...
  function createChatCard(persona, message, timestamp, replyTo=null) {
    const badgeHtml = persona.admin ? '<span class="chat-msg-badge badge-admin">Admin</span>' : (persona.moderator ? '<span class="chat-msg-badge badge-vip" style="background:rgba(59,130,246,.14);color:#60a5fa;">Mod</span>' : (persona.vip>0 ? `<span class="chat-msg-badge badge-vip">VIP ${persona.vip}</span>` : ''));
    let avatarHtml;
    if (persona.admin) avatarHtml = `<img src="${persona.avatar}" alt="Admin" style="width:36px;height:36px;border-radius:50%;object-fit:cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="chat-msg-avatar" style="display:none;background:rgba(245,158,11,.22)">${persona.initials}</div>`;
    else if (persona.moderator||persona.isFallback) avatarHtml = `<img src="${persona.avatar}" alt="${persona.name}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="chat-msg-avatar" style="display:none;background:${persona.moderator?'rgba(59,130,246,.22)':'rgba(168,85,247,.22)'}">${persona.initials}</div>`;
    else avatarHtml = `<img src="${persona.avatar}" alt="${persona.name}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="chat-msg-avatar" style="display:none;background:rgba(168,85,247,.22)">${persona.initials}</div>`;
    const timeAgo = Math.floor((Date.now()-timestamp)/60000);
    const relativeTime = formatRelativeTime(timeAgo);
    let replyHtml = '';
    if (replyTo) replyHtml = `<div style="background:rgba(168,85,247,.04); border-left:3px solid #a855f7; border-radius:6px; padding:8px 10px; margin-bottom:10px; font-size:12px; color:#9ca3af;"><div style="font-weight:400; color:#c084fc; margin-bottom:4px;">↳ Replying to ${replyTo.persona.name}</div><div style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100%;">${replyTo.message}</div></div>`;
  ...
  // ========== UNREAD BADGE (scroll‑event based) ==========
  window.unreadCount = 0;
  window.latestChatCard = null;
  let isNearTop = true;  // true when scroll is at the top

  function updateUnreadBadge() {
    const badge = document.getElementById('unreadBadge');
    if (!badge) return;
    if (window.unreadCount > 0) {
      badge.textContent = `↓ ${window.unreadCount} new`;
  ...
  function handleScroll() {
    ...
    if (!col) return;
    isNearTop = col.scrollTop < 20;   // within 20px of the top = viewing newest
  }
  ...
  function addRandomChatMessage() {
    const col = document.getElementById('communityChatColumn');
    if (!col) return;
    // Only add messages if community tab is active
    const communityView = document.getElementById('communityView');
    if (!communityView || !communityView.classList.contains('active')) return;
    ...
    if (Math.random() < 0.4) computedTime += randomInt(600,2500);
    ...
    function _actuallyAddMessage(persona, message) {
      ...
      if (Math.random() < 0.12 && persona.traits.archetype !== 'lurker') {
        ...
        setTimeout(() => {
          ...
          if (cardElement2) { startReactionSimulation(cardElement2); scrollToCard(cardElement2); }
          while (col.children.length > 80) col.removeChild(col.lastChild);
  ...
  const currencyMethods = {
    ...
    CAD: [
      { name: 'RBC Royal Bank', icon: 'assets/banks/rbc.png' },
      { name: 'TD Canada Trust', icon: 'assets/banks/td.png' },
      { name: 'Scotiabank', icon: 'assets/banks/scotiabank.png' },
      { name: 'BMO', icon: 'assets/banks/bmo.png' },
      { name: 'CIBC', icon: 'assets/banks/cibc.png' }
    ],
    JPY: [
      { name: 'MUFG Bank', icon: 'assets/banks/mufg.png' },
      { name: 'Sumitomo Mitsui', icon: 'assets/banks/smfg.png' },
      { name: 'Mizuho Bank', icon: 'assets/banks/mizuho.png' },
      { name: 'Japan Post Bank', icon: 'assets/banks/japanpost.png' },
      { name: 'Resona Bank', icon: 'assets/banks/resona.png' }
    ],
    AUD: [
      { name: 'Commonwealth Bank', icon: 'assets/banks/commbank.png' },
      { name: 'ANZ', icon: 'assets/banks/anz.png' },
      { name: 'Westpac', icon: 'assets/banks/westpac.png' },
      { name: 'NAB', icon: 'assets/banks/nab.png' },
      { name: 'Bank of Queensland', icon: 'assets/banks/boq.png' }
    ]
  };
  const currencySymbols = { NGN: '₦', USD: '$', EUR: '€', GBP: '£', USDT: 'USDT', CAD: 'C$', JPY: '¥', AUD: 'A$' };
  const toUSDT = { USDT: 1, USD: 1, EUR: 1.08, GBP: 1.27, NGN: 0.000667, CAD: 0.73, JPY: 0.0067, AUD: 0.65 };
  function getCurrentCurrency() { const btn = document.getElementById('currencyBtn'); if (!btn) return 'USDT'; return btn.textContent.trim().split(' ')[0]; }
  function formatAmount(amount, currency) { const sym = currencySymbols[currency] || ''; return sym + amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  function maskName(fullName) { const clean = fullName.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').trim(); if (clean.length <= 2) return clean[0] + '*'.repeat(Math.max(1, clean.length - 1)); const first = clean[0], last = clean[clean.length - 1]; return first + '*'.repeat(clean.length - 2) + last; }
  function timeAgo(date) { const seconds = Math.floor((Date.now() - date.getTime()) / 1000); if (seconds < 10) return 'Just now'; if (seconds < 60) return `${seconds} sec ago`; const mins = Math.floor(seconds / 60); if (mins < 60) return `${mins} min ago`; const hours = Math.floor(mins / 60); return `${hours} hour${hours > 1 ? 's' : ''} ago`; }
  ...
  function createWithdrawal(persona, minutesAgo, currency) {
    ...
    return { id: Date.now() + Math.random(), maskedName: maskName(persona.name), initials: persona.initials, avatarUrl: persona.avatar, isFallback: persona.isFallback, amount, currency, methodName: method.name, methodIcon: method.icon, time: date, status: 'completed' };
  }
  let allWithdrawals = [];
  const currencies = ['NGN', 'USD', 'EUR', 'GBP', 'USDT', 'CAD', 'JPY', 'AUD'];
  currencies.forEach(cur => { for (let i = 0; i < 50; i++) { const persona = shuffledPool[(currencies.indexOf(cur) * 50 + i) % shuffledPool.length]; allWithdrawals.push(createWithdrawal(persona, Math.floor(Math.random() * 1440) + 1, cur)); } });
  ...
  function renderFilterTabsInline() {
    ...
    container.innerHTML = `
      ...
      <button class="liveFilterTabInline" data-filter="cad" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:6px 14px; border-radius:999px; font-size:12px; cursor:pointer; white-space:nowrap;">CAD</button>
      <button class="liveFilterTabInline" data-filter="jpy" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:6px 14px; border-radius:999px; font-size:12px; cursor:pointer; white-space:nowrap;">JPY</button>
      <button class="liveFilterTabInline" data-filter="aud" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:6px 14px; border-radius:999px; font-size:12px; cursor:pointer; white-space:nowrap;">AUD</button>`;
  ...
  function avatarHTML(w) { return `<div style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg, #a855f7, #6d28d9); display:grid; place-items:center; font-size:14px; font-weight:700; color:white;">${w.initials}</div>`; }
  function renderLiveWithdrawalListInline() {
    ...
    else if (inlineActiveFilter === 'cad') filtered = filtered.filter(w => w.currency === 'CAD');
    else if (inlineActiveFilter === 'jpy') filtered = filtered.filter(w => w.currency === 'JPY');
    else if (inlineActiveFilter === 'aud') filtered = filtered.filter(w => w.currency === 'AUD');
  ...
  function getUserAnnouncementDates() {
    ...
    const now = new Date();
    let firstVisit;
    if (user.firstVisitDate) { const parsed = new Date(user.firstVisitDate); firstVisit = !isNaN(parsed.getTime()) ? parsed : now; } else firstVisit = now;
  ...
  function switchToGeneralChat() {
    ...
    const generalTab = document.getElementById('tabGeneralChat'); if (generalTab) generalTab.classList.add('active');
    const chatLayout = document.getElementById('communityChatLayout'); if (chatLayout) chatLayout.style.display = '';
    const winsContent = document.getElementById('winsProofsContent'); if (winsContent) winsContent.style.display = 'none';
    const announcementsContainer = document.getElementById('announcementsContent'); if (announcementsContainer) announcementsContainer.style.display = 'none';
    const helpDeskContainer = document.getElementById('helpDeskContent'); if (helpDeskContainer) helpDeskContainer.style.display = 'none';
    const membersPanel = document.getElementById('communityMembersPanel'); if (membersPanel) membersPanel.style.display = 'none';
    const pinned = document.querySelector('.community-pinned'); if (pinned) pinned.style.display = 'flex';
    const inputBar = document.getElementById('communityInputBar'); if (inputBar) inputBar.style.display = 'flex';
  }
  function switchToWinsProofs() {
    const pinned = document.querySelector('.community-pinned'); if (pinned) pinned.style.display = 'none';
    const inputBar = document.getElementById('communityInputBar'); if (inputBar) inputBar.style.display = 'none';
    const announcementsContainer = document.getElementById('announcementsContent'); if (announcementsContainer) announcementsContainer.style.display = 'none';
    const helpDeskContainer = document.getElementById('helpDeskContent'); if (helpDeskContainer) helpDeskContainer.style.display = 'none';
    const membersPanel = document.getElementById('communityMembersPanel'); if (membersPanel) membersPanel.style.display = 'none';
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    const winsTab = document.getElementById('tabWinsProofs'); if (winsTab) winsTab.classList.add('active');
    const chatLayout = document.getElementById('communityChatLayout'); if (chatLayout) chatLayout.style.display = 'none';
    const winsContent = document.getElementById('winsProofsContent'); if (winsContent) winsContent.style.display = 'block';
    buildWinsProofsContent(); updateHeroAvatarStackInline(); renderFilterTabsInline();
    inlineActiveFilter = 'all'; inlineLoadedCount = 6; renderLiveWithdrawalListInline();
  ...
  function switchToMarketUpdates() {
    ...
    const membersPanel = document.getElementById('communityMembersPanel'); if (membersPanel) membersPanel.style.display = 'none';
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    const tab = document.getElementById('tabMarketUpdates'); if (tab) tab.classList.add('active');
    renderAnnouncements();
    const announcementsContainer = document.getElementById('announcementsContent'); if (announcementsContainer) announcementsContainer.style.display = 'block';
  }
  function switchToHelpDesk() {
    ...
    const membersPanel = document.getElementById('communityMembersPanel'); if (membersPanel) membersPanel.style.display = 'none';
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    const tab = document.getElementById('tabHelpDesk'); if (tab) tab.classList.add('active');
    renderHelpDesk();
    const helpDeskContainer = document.getElementById('helpDeskContent'); if (helpDeskContainer) helpDeskContainer.style.display = 'block';
  }
  ...
  function injectNewLiveWithdrawal() {
    const cur = getCurrentCurrency();
    const persona = shuffledPool[poolIndex % shuffledPool.length]; poolIndex++;
  ...
  function startCommunitySimulation() {
    try {
      ...
      // Typing sound observer (attach only after indicator exists)
      ...
      typingObserver.observe(typingDiv, { attributes: true, attributeFilter: ['style'] });

      // Force typing test after 5s (optional)
      setTimeout(() => {
      ...
      badge.style.zIndex = '10000';   // ensure visibility
      ...
      setInterval(() => {
        if (!communityView.classList.contains('active')) return;
        updateOnlineMembers(); updateSidebarWithdrawals();
      }, 45000);
  ...
  // ========== PERSISTENT DOWNLOAD APP PILL (replaces referral banner) ==========
  (function createDownloadPill() {
    const pill = document.createElement('button');
    pill.id = 'pwaCommunityPill';
    pill.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 21h14"/></svg> Download App`;
    ...
    const col = document.getElementById('communityChatColumn');
    if (col) {
      col.parentNode.insertBefore(pill, col);
    }

    // Sync with the global deferredPrompt from dashboard.html
    ...
    pill.addEventListener('click', async () => {
      if (window.deferredPWA) {
        ...
        if (outcome === 'accepted') {
          pill.style.display = 'none';  // hide after install
        }
        window.deferredPWA = null;
      } else {
        alert('To install the app, use your browser menu (e.g., "Add to Home Screen").');
      }
    });
