// ===== Trade Pulse AI – community.js (FULL with fixed announcement dates – DEFINITIVE) =====
(function() {
  ...
  function getUserAnnouncementDates() {
    ...
    // Always regenerate if dates are missing or invalid
    if (
      user.announcementDates &&
      Array.isArray(user.announcementDates) &&
      user.announcementDates.length === 12
    ) {
      const allValid = user.announcementDates.every(
        d => typeof d.postedStr === 'string' && d.postedStr.length > 0
      );
    ...
    // Calculate next Friday after firstVisit
    const nextFriday = new Date(firstVisit);
    nextFriday.setDate(
      firstVisit.getDate() + ((5 - firstVisit.getDay() + 7) % 7) + 7
    );
    ...
    for (let i = 0; i < 12; i++) {
      if (i === 0) {
        ...
        const postedStr =
          ...
          ' · ' +
          postedDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
        dates.push({
          id: 1,
          postedDate: postedDate,
          closingDate: nextFriday,
          postedStr: postedStr
        });
      } else {
        const postedDate = new Date(now);
