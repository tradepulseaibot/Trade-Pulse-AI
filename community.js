// ===== Trade Pulse AI – community.js (FULLY HUMANIZED) =====
(function() {
  console.log('community.js loaded (humanized v2)');
  ...
  function startReactionSimulation(cardElement) {
    ...
    // Delayed start for more organic feel – first reaction appears after 1-3 seconds
  ...
  // ========== CHAT PACE: add messages gradually (with typing indicator, reply preview, double posts) ==========
  ...
  function addRandomChatMessage() {
    ...
    const showTyping = Math.random() < 0.3;
    ...
    function _actuallyAddMessage(persona, message) {
      ...
      // Human-like double post (15% chance after a short pause)
      if (Math.random() < 0.15 && persona.traits.archetype !== 'lurker') {
        const followUpDelay = 2000 + Math.random() * 4000; // 2-6 seconds
        setTimeout(() => {
  ...
  function injectJoinNotification() {
    ...
    // No sound for join notifications
  }
  ...
  function startCommunitySimulation() {
    try {
      ...
      typingDiv.style.background = 'linear-gradient(180deg, rgba(11,16,32,.95), rgba(7,10,20,.98))';
      ...
      typingDiv.style.border = '1px solid rgba(255,255,255,.05)';

      const communityChatColumn = document.getElementById('communityChatColumn');
      if (communityChatColumn) {
        communityChatColumn.insertBefore(typingDiv, communityChatColumn.firstChild);
      }
