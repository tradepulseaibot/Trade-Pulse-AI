// ===== Trade Pulse AI – community.js (FULL – debug panel guaranteed) =====
(function() {
  console.log('community.js loaded');

  // ──────────────────────────────────────────────
  //  DEBUG PANEL – created immediately, even if simulation fails
  // ──────────────────────────────────────────────
  function createDebugPanel() {
    ...
    panel.innerHTML = `
      <div style="background:#0a0f1f; color:#e5e7eb; border-top:2px solid #a855f7; font-family:monospace; font-size:11px;">
        <div style="display:flex; align-items:center; justify-content:space-between; padding:4px 8px; background:#111827;">
          <span style="color:#a855f7; font-weight:700;">🐞 Debug</span>
          <div>
            <button id="debugCopyBtn" style="background:#1f2937; border:1px solid #374151; color:#d1d5db; padding:2px 8px; border-radius:4px; cursor:pointer; font-size:10px;">📋 Copy</button>
            <button id="debugToggleBtn" style="background:#1f2937; border:1px solid #374151; color:#d1d5db; padding:2px 8px; border-radius:4px; cursor:pointer; font-size:10px;">▲</button>
          </div>
        </div>
        <div id="debugLogContainer" style="max-height:150px; overflow-y:auto; padding:4px 8px; white-space:pre-wrap; word-break:break-all;"></div>
      </div>`;
    panel.style.cssText = 'position:fixed; bottom:0; left:0; right:0; z-index:99999;';
    ...
    console.log = function(...a) { origLog.apply(console,a); addLog('LOG',a); };
    console.warn = function(...a) { origWarn.apply(console,a); addLog('WARN',a); };
    console.error = function(...a) { origError.apply(console,a); addLog('ERROR',a); };
    ...
    window.addEventListener('unhandledrejection', function(ev) {
      const reason = ev.reason ? (ev.reason.stack || ev.reason.message || ev.reason) : 'Unknown';
      origError(reason);
    ...
    toggleBtn.addEventListener('click', ()=>{
      logContainer.style.display = visible ? 'none' : '';
