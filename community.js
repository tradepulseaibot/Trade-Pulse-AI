// ===== Trade Pulse AI – community.js (FULL VERSION with organic chat refinements) =====
(function() {
  ...
  const archetypeTraits = {
    ...
    moderator:  { archetype: "moderator", grammar: "clean", slang: 0.1, typoRate: 0.01 }  // professional, friendly
  };
  ...
  const personaNames = personaData.map(p => {
    ...
    if (p.isModerator) {
      ...
      avatar = `assets/avatars/${safe}.jpg`;  // attempt custom avatar, onerror will fallback to initials
    } else if (isFallback) {
  ...
  function getRandomMessage(persona) {
    // Moderators get professional yet friendly messages, no slang/typos
  ...
  function getUserAnnouncementDates() {
    ...
    // For ID 1: closing date = next week's Friday from first visit
    ...
    for (let i = 0; i < 12; i++) {
      if (i === 0) {
        // posted date for ID 1: random past 1-3 days
        ...
        dates.push({ id: 1, postedDate: postedDate, closingDate: nextFriday, postedStr: postedStr });
      } else {
        const postedDate = new Date(now);
        postedDate.setDate(postedDate.getDate() - (Math.floor(Math.random() * 7) + 1));
        const postedStr = postedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ' · ' + postedDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
        dates.push({ id: i+1, postedDate: postedDate, closingDate: null, postedStr: postedStr });
      }
    }
  ...
  function renderAnnouncements() {
    ...
    const getDateForId = (id) => {
      ...
      return found ? found.postedStr : 'Unknown date';
    };
