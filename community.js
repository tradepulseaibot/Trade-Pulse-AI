// ===== Trade Pulse AI – community.js (FULL – typing indicator debugged, forced test, debug panel) =====
(function() {
  console.log('community.js loaded (humanized v7)');
  ...
  function trackRecentMessage(persona, message) {
    ...
    if (recentMessageObjects.length > MAX_RECENT_FOR_REPLY) {
      recentMessageObjects.shift();
    }
  }

  // ========== CHAT SOUND (only when community tab is active) ==========
  ...
  // ========== ORGANIC REACTION SYSTEM (delayed) ==========
  ...
  function createChatCard(persona, message, timestamp, replyTo = null) {
    ...
    if (replyTo) {
      replyHtml = `
        <div style="background:rgba(168,85,247,.04); border-left:3px solid #a855f7; border-radius:6px; padding:8px 10px; margin-bottom:10px; font-size:12px; color:#9ca3af;">
  ...
  // ========== CHAT PACE (typing indicator 70%, forced test) ==========
  ...
  function addRandomChatMessage() {
    ...
    const maxTypingTime = 5000;
    const computedTime = messageLength * TYPING_MS_PER_CHAR;
    ...
    if (showTyping) {
      ...
      console.log('Typing indicator triggered for', persona.name, '| delay:', typingDelay);
      if (typingDiv) {
    ...
    function _actuallyAddMessage(persona, message) {
      ...
      if (Math.random() < 0.15 && persona.traits.archetype !== 'lurker') {
        ...
        setTimeout(() => {
          ...
          col.insertAdjacentHTML('afterbegin', cardHtml2);
          const cardElement2 = col.firstElementChild;
  ...
  function injectJoinNotification() {
    ...
    col.insertAdjacentHTML('afterbegin', joinCard.outerHTML);
  }
  ...
  // ========== COMMUNITY DEBUG PANEL ==========
  function createCommunityDebugPanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="communityDebugLogContainer" style="overflow-y:auto; padding:4px 8px; max-height:25vh; white-space:pre-wrap; word-break:break-all;"></div>
    `;
    ...
    addLog('DEBUG', ['Community debug panel active.']);
    addLog('INFO', ['Typing indicator element present: ' + (!!document.getElementById('typingIndicator'))]);
    addLog('INFO', ['soundTyping audio present: ' + (!!document.getElementById('soundTyping'))]);
  }
  ...
  function startCommunitySimulation() {
    try {
      ...
      // Create typing indicator (styled to stand out)
      ...
      // FORCE a typing indicator after 5 seconds to confirm it works
      setTimeout(() => {
        const forceDiv = document.getElementById('typingIndicator');
        if (forceDiv) {
          ...
          console.log('Forced typing indicator shown.');
        }
      }, 5000);

      createCommunityDebugPanel();
