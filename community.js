// ===== Trade Pulse AI – community.js (FINAL – all fixes, no proof cards) =====
(function() {
  ...
  const currencySymbols = { NGN: '₦', USD: '$', EUR: '€', GBP: '£', USDT: '₮', CAD: 'C$', JPY: '¥', AUD: 'A$' };
  ...
  (function createDownloadPill() {
    ...
    pill.addEventListener('click', async () => {
      if (window.deferredPWA) {
        ...
        alert('App is already installed or not supported on this device.');
      }
    });
  ...
  // ========== TYPING SOUND OBSERVER ==========
  (function ensureTypingAudio() {
    if (document.getElementById('soundTyping')) return;
    const audio = document.createElement('audio');
    audio.id = 'soundTyping';
    audio.src = 'assets/sounds/typing.mp3';
    audio.preload = 'auto';
    document.body.appendChild(audio);
  })();
  const typingObserver = new MutationObserver(mutations => {
    mutations.forEach(m => {
      if (m.target.id === 'typingIndicator' && m.target.style.display === 'block') {
        const typingSound = document.getElementById('soundTyping');
        if (typingSound) { typingSound.currentTime = 0; typingSound.play().catch(()=>{}); }
      }
    });
  });
  const typingDivObserve = document.getElementById('typingIndicator');
  if (typingDivObserve) typingObserver.observe(typingDivObserve, { attributes: true, attributeFilter: ['style'] });
