(function() {
  ...
  // Build persona objects with avatars
  const personaNames = personaData.map(p => {
    ...
    if (p.isModerator) {
      ...
      avatar = `assets/avatars/${safe}.jpg`;  // attempt custom avatar, fallback to initials on error
    } else if (isFallback) {
  ...
  // Add admin persona at the beginning
  ...
  function getRandomMessage(persona) {
    // Moderators get professional yet friendly messages, no slang/typos, from advice/hype/testimonial/general
    ...
    // Emoji only for high-slang personas, not every message
  ...
  // Pre-load only 5-8 messages initially
  ...
  // ========== ANNOUNCEMENTS WITH USER-SPECIFIC DATES ==========
  ...
  function getUserAnnouncementDates() {
    ...
    if (!user) return null;
    if (user.announcementDates) return user.announcementDates;
    
    const now = new Date();
    const firstVisit = user.firstVisitDate ? new Date(user.firstVisitDate) : now;
    const nextFriday = new Date(firstVisit);
    nextFriday.setDate(firstVisit.getDate() + ((5 - firstVisit.getDay() + 7) % 7) + 7); // next week's Friday
    const closingDateStr = nextFriday.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    
    const dates = [];
    for (let i = 0; i < 12; i++) {
      if (i === 0) {
        dates.push({ id: 1, dateStr: closingDateStr, fullDate: nextFriday });
      } else {
        const daysAgo = Math.floor(Math.random() * 14) + 7;
        const d = new Date(now);
        d.setDate(d.getDate() - daysAgo);
        dates.push({ id: i+1, dateStr: d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ' · ' + d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }), fullDate: d });
      }
    }
  ...
  function renderAnnouncements() {
    ...
    const getDateForId = (id) => {
      ...
      return found ? found.dateStr : 'Unknown date';
    };

    // Admin avatar with image and fallback initials
    ...
    container.innerHTML = `
      <div class="section" style="margin-top:0;">
        ...
        ${announcementsTemplate.map(a => {
          const desc = a.id === 1 ? a.description.replace(/closes on .+$/, `closes on ${getDateForId(a.id)}.`) : a.description;
          return `
