// ===== Trade Pulse AI – community.js (FULL – all features, typing indicator stable, scroll fix, realistic typing, more joiners, NO debug panel) =====
(function() {
  console.log('community.js loaded (humanized v10)');
  ...
  function createProofChatCard(proofPersona) {
    ...
    return `<div class="community-chat-card" data-msg-timestamp="${Date.now()}" style="overflow:visible; display:block; min-height:fit-content; word-break:break-word;">
  ...
  function createChatCard(persona, message, timestamp, replyTo = null) {
    ...
    if (replyTo) {
      replyHtml = `<div style="background:rgba(168,85,247,.04); border-left:3px solid #a855f7; border-radius:6px; padding:8px 10px; margin-bottom:10px; font-size:12px; color:#9ca3af;">
        <div style="font-weight:600; color:#c084fc; margin-bottom:4px;">↳ Replying to ${replyTo.persona.name}</div>
        <div style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100%;">${replyTo.message}</div>
      </div>`;
    }

    return `<div class="community-chat-card" data-msg-timestamp="${timestamp}" style="overflow:visible; display:block; min-height:fit-content; word-break:break-word;">
  ...
  function addRandomChatMessage() {
    ...
    if (Math.random() < 0.4) computedTime += randomInt(500, 2000);
  ...
  function avatarHTML(w) {
    return `<div style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg, #a855f7, #6d28d9); display:grid; place-items:center; font-size:14px; font-weight:700; color:white;">${w.initials}</div>`;
  }
  ...
  // ========== ANNOUNCEMENTS ==========
  // ... (same as previous full version, unchanged)
  ...
  function renderHelpDesk() {
    ...
    container.innerHTML = `<div class="section" style="margin-top:0;"><div style="background:rgba(11,16,32,.9); border-radius:20px; padding:24px; text-align:center;"><div style="font-size:48px; margin-bottom:16px;">🎧</div><div style="font-size:20px; font-weight:700; margin-bottom:8px;">24/7 Support</div><div style="font-size:14px; color:#9ca3af; margin-bottom:20px;">Our support team is available around the clock via Telegram. Tap the button below to chat with us instantly.</div><a href="https://t.me/trade_pulse_ai_support" target="_blank" style="display:inline-block; background:linear-gradient(90deg, #6d28d9, #8b5cf6); color:#fff; padding:14px 28px; border-radius:999px; font-size:16px; font-weight:700; text-decoration:none; box-shadow:0 6px 18px rgba(124,58,237,.2);"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:middle; margin-right:8px;"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.07-.2-.08-.06-.19-.04-.27-.02-.12.02-1.96 1.25-5.54 3.66-.52.36-1 .53-1.42.52-.47-.01-1.37-.26-2.03-.48-.82-.27-1.47-.42-1.41-.88.03-.24.37-.49 1.02-.74 4.01-1.75 6.69-2.9 8.05-3.45 3.84-1.6 4.64-1.88 5.16-1.89.11 0 .37.03.54.16.14.11.18.26.2.37.02.11.04.37.03.57z"/></svg>Contact Support on Telegram</a></div></div>`;
  }
