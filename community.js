// ===== Trade Pulse AI – community.js (FULL with withdrawal proof messages, increased frequency) =====
(function() {
  ...
  function startCommunitySimulation() {
    try {
      ...
      // ---- WITHDRAWAL PROOF MESSAGES (increased frequency) ----
      resetProofCycle();

      setInterval(() => {
        if (!communityView.classList.contains('active')) return;
        const now = Date.now();
        if (now - lastProofTimestamp < 30000) return; // 30‑second cooldown
        if (Math.random() > 0.4) return; // 60% chance → about 1‑2 proofs per minute
