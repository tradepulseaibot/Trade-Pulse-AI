// ===== Trade Pulse AI – community.js (FULL – debug panel, all features) =====
(function() {
  console.log('community.js loaded (humanized v14 – debug panel)');

  // ========== DEBUG PANEL ==========
  function createDebugPanel() {
    if (document.getElementById('communityDebugPanel')) return;

    const panel = document.createElement('div');
    panel.id = 'communityDebugPanel';
    panel.style.position = 'fixed';
    panel.style.bottom = '0';
    panel.style.left = '0';
    panel.style.right = '0';
    panel.style.maxHeight = '25vh';
    panel.style.background = '#0a0f1f';
    panel.style.color = '#e5e7eb';
    panel.style.borderTop = '2px solid #a855f7';
    ...
    panel.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; padding:4px 8px; background:#111827; border-bottom:1px solid #1f2937;">
        <span style="color:#a855f7; font-weight:700;">🐞 Community Debug</span>
        <div style="display:flex; gap:6px;">
          <button id="debugCommunityCopyBtn" style="background:#1f2937; border:1px solid #374151; color:#d1d5db; padding:2px 8px; border-radius:4px; cursor:pointer; font-size:10px;">📋 Copy</button>
          <button id="debugCommunityToggleBtn" style="background:#1f2937; border:1px solid #374151; color:#d1d5db; padding:2px 8px; border-radius:4px; cursor:pointer; font-size:10px;">▲</button>
        </div>
      </div>
      <div id="communityDebugLogContainer" style="overflow-y:auto; padding:4px 8px; max-height:22vh; white-space:pre-wrap; word-break:break-all;"></div>
    `;
    document.body.appendChild(panel);

    const logContainer = document.getElementById('communityDebugLogContainer');
    const copyBtn = document.getElementById('debugCommunityCopyBtn');
    const toggleBtn = document.getElementById('debugCommunityToggleBtn');
    let logLines = [];
    const MAX_LINES = 500;
    ...
    console.log = function(...args) { origLog.apply(console, args); addLog('LOG', args); };
    ...
    window.onerror = function(message, source, lineno, colno, error) {
      ...
      origError(msg);
      addLog('GLOBAL_ERROR', [msg]);
      return true;
    };

    window.addEventListener('unhandledrejection', function(event) {
      const reason = event.reason ? (event.reason.stack || event.reason.message || event.reason) : 'Unknown';
      const msg = `Unhandled Rejection: ${reason}`;
      origError(msg);
      addLog('UNHANDLED_REJECTION', [msg]);
    });

    let panelVisible = true;
    toggleBtn.addEventListener('click', () => {
      const container = document.getElementById('communityDebugLogContainer');
      if (panelVisible) {
    ...
    addLog('INFO', ['Debug panel active.']);
  }
  ...
  function createChatCard(persona, message, timestamp, replyTo = null) {
    ...
    console.log('createChatCard called for', persona.name, 'message length', message.length, 'reply', !!replyTo);
  ...
  // ========== UNREAD BADGE & SCROLL LOGIC ==========
  window.unreadCount = 0;
  window.latestChatCard = null;

  function updateUnreadBadge() {
    const badge = document.getElementById('unreadBadge');
    console.log('updateUnreadBadge - count:', window.unreadCount, 'badge exists:', !!badge);
    if (!badge) return;
  ...
  function scrollToLatest() {
    console.log('scrollToLatest called, latestChatCard:', window.latestChatCard);
  ...
  function scrollToCard(cardElement) {
    ...
    console.log('scrollToCard - scrollTop:', col.scrollTop, 'isNearTop:', isNearTop);
    if (isNearTop) {
      cardElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
    } else {
      window.unreadCount++;
      window.latestChatCard = cardElement;
      console.log('Increment unread count to', window.unreadCount);
      updateUnreadBadge();
    }
  }
  ...
  function insertChatCard(cardHtml) {
    ...
    if (!col) return null;
    const indicator = window.typingIndicatorEl;
    console.log('insertChatCard - col:', !!col, 'indicator:', !!indicator, 'indicator parent:', indicator ? indicator.parentNode === col : false);
    if (indicator && indicator.parentNode === col) {
  ...
  function addRandomChatMessage() {
    console.log('addRandomChatMessage called');
    const col = document.getElementById('communityChatColumn');
    if (!col) return;
    const persona = getRandomPersona();
    const message = getRandomMessage(persona);
    ...
    if (showTyping && typingDiv) {
      ...
      // Human-like: sometimes (15%) cancel the message after typing a bit
      if (Math.random() < 0.15) {
        const cancelDelay = Math.min(typingDelay * 0.4, 3000);
        setTimeout(() => { typingDiv.style.display = 'none'; console.log('Typing cancelled'); }, cancelDelay);
      } else {
        setTimeout(() => {
    ...
    function _actuallyAddMessage(persona, message) {
      console.log('_actuallyAddMessage for', persona.name);
      let replyTo = null;
      ...
      if (cardElement) {
        ...
        console.warn('cardElement is null after insertChatCard');
      }
  ...
  function startCommunitySimulation() {
    try {
      createDebugPanel();
      console.log('startCommunitySimulation called');
      const communityView = document.getElementById('communityView');
      if (!communityView) { console.error('communityView missing'); return; }
      const col = document.getElementById('communityChatColumn');
      if (!col) { console.error('communityChatColumn missing'); return; }
      ensureChatScrollable();
      console.log('Chat column found, scrollable set up');
      ...
      // Force typing test after 5 seconds
      setTimeout(() => {
        ...
        if (forceDiv) {
          ...
          console.log('Force typing indicator shown');
        }
      }, 5000);

      // ─── UNREAD BADGE (inside column, right after typing indicator) ───
      ...
      col.insertBefore(badge, typingDiv.nextSibling);
      console.log('Unread badge inserted into chat column');
      ...
      console.error('startCommunitySimulation error:', error);
    }
  }
