// ===== Trade Pulse AI – community.js (FULL – all features, unread badge, human tweaks, NO debug panel) =====
(function() {
  console.log('community.js loaded (humanized v12)');
  ...
  const adminPersona = {
    name: "Aditya Renash", gender: "men", country: "India", isFallback: false, vip: 0, admin: true,
    initials: "AR", avatar: "assets/avatars/aditya_renash.jpg",
  ...
  const personaNames = personaData.map(p => {
    ...
    if (p.isModerator) {
      ...
      const safe = p.name.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').replace(/[^\w\s]/g, '').trim().replace(/\s+/g, '_').toLowerCase();
      avatar = `assets/avatars/${safe}.jpg`;
    }
    const cleanInitials = p.name.split(' ').map(part => part.replace(/[^A-Za-z]/g, '').charAt(0)).join('').toUpperCase();
    return {
  ...
  function trackRecentMessage(persona, message) { recentMessageObjects.push({ persona, message, timestamp: Date.now() }); if (recentMessageObjects.length > MAX_RECENT_FOR_REPLY) recentMessageObjects.shift(); }
  ...
  function createProofChatCard(proofPersona) {
    ...
    return `<div class="community-chat-card" data-msg-timestamp="${Date.now()}" style="overflow:visible; display:block; min-height:fit-content; flex-shrink:0; word-break:break-word;">
      ...
      <div class="chat-msg-body" style="word-break:break-word; white-space:pre-wrap; overflow-wrap:break-word;">${message}</div>
  ...
  const messageCategories = {
    general: [
      "Good morning everyone. Hope you're all profitable today.", "Market looking interesting today. Who's ready for the session?",
      "Trading is a marathon, not a sprint.", "Stay disciplined, stay profitable.", "I love the energy in this community. Let's keep winning.",
      "Consistency beats luck. Stick to your plan.", "Another green day loading… I can feel it.",
      "Don't forget to check the charts before opening trades.", "Patience is the trader's superpower.",
      "Let's make this week our best one yet.", "Risk management is the real key to success.",
      "Every pro was once a beginner. Keep learning!", "Let's share the wins and learn from the losses.",
      "Anyone else trading the New York session?", "Quiet morning for me – waiting for the right setup.",
      "I'm still learning, but loving the process.", "This platform makes trading so much easier.",
      "Happy to be part of a community that shares real knowledge.", "Let's keep pushing forward. No looking back!",
      "New day, new opportunities. Let's get after it.", "Just woke up and already see green pips. What a day!",
      "Who else is holding through the weekend?", "Never a dull moment in this market. Love it.",
      "I'm genuinely impressed by how supportive everyone is.", "Wishing everyone a profitable session ahead.",
      "London open in a few – who's ready?", "Slow start today, but I'm staying patient.",
      "Just finished my morning analysis – things look promising.", "Stay humble when you're winning, and calm when you're not.",
      "I'm bullish on this community. So much good vibes!", "Checking in from Nigeria – everything steady over here.",
      "Sending good vibes to all traders tonight.", "Focus on the process, not just the profit.",
      "Who else finds the pre‑market the most exciting?", "Just catching up – anything big happen overnight?"
    ],
    question: [
      "How long does withdrawal take usually?", "What's the minimum deposit to get started?", "Can I use a debit card for deposits?",
      "Is USDT available for withdrawals?", "How do I set a stop loss properly?", "What's the difference between OTC and live trading?",
      "Do you offer copy trading?", "How often are signals sent out?", "Is this AI‑generated signals or manual?",
      "I'm new – any tips for my first trade?", "Which session gives the best moves for beginners?",
      "Do you trade around news events?", "What risk percentage do you recommend per trade?",
      "How many pips do you usually target?", "Can I try a demo account first?", "How do I verify my account?",
      "Is there a mobile app for this?", "What's the success rate of the signals?", "Do you support bank transfers?",
      "How long does it take to learn the basics?", "Are there any hidden fees when I withdraw?",
      "Can I trade on weekends?", "What's the best pair to focus on as a beginner?", "Do you have a referral program?",
      "When is the best time to enter a position?", "How secure is my personal data?", "Can I have multiple accounts?",
      "What's the leverage offered?", "Is there a maximum I can invest?", "How quickly do signals update?",
      "Do you offer any educational resources?", "What's the difference between Standard and Premium plans?",
      "How often are the plans updated?", "Can I change my investment amount after starting?",
      "Is there a withdrawal limit per day?", "Do I need to keep my account active?", "Will I get notifications when a signal opens?",
      "What happens if I miss a signal?", "Can I talk to a real person for help?"
    ],
    hype: [
      "This thing too sweet 🔥", "Just closed another winner!", "We eating good today 💰", "Pips for days!",
      "Market is giving today – don't miss it!", "Account growing daily. Consistency is everything.",
      "Compounding is the secret sauce. Let's go!", "Making the market my ATM!",
      "Don't sleep on these signals – they're on fire!", "I'm telling you, this works! Stick with it.",
      "Easy money when you follow the plan.", "Let's get this bread! 🥖", "Who else caught that move? I'm up big.",
      "I called it yesterday – today it played out perfectly.", "Wait till you see my next trade. It's going to be huge!",
      "The haters will be quiet after this one!", "No losses today. Perfect execution.",
      "I'm literally dancing right now. Profits rolling in!", "My heart is racing – just hit my personal best!",
      "This community is full of winners. Prove me wrong!", "I just hit 50% return this month. Insane!",
      "The signals are so accurate it's almost scary.", "From $100 to $500 in two weeks – I'm speechless.",
      "I've never felt this confident trading before.", "The VIP signals are on another level.",
      "My withdrawal just landed. Time to celebrate!", "I'm not even watching the charts – the AI does it all.",
      "This is the best decision I've made all year.", "Let's crush the markets together!",
      "I'm so hyped I can't sleep. Another big day tomorrow.", "My wife thinks I'm a genius now 🤣",
      "My friends keep asking me how I do it. Trade Pulse AI!", "I just cleared my debts thanks to this platform.",
      "If you're not in yet, you're missing out big time.", "Signal accuracy is off the charts this month.",
      "I'm up 300% since I started. Not kidding.", "Reinvesting profits and watching it compound – beautiful.",
      "The admins here are literally angels.", "Who needs a 9‑5 when you have Trade Pulse?",
      "Every day I wake up richer. Thank you signals!", "This community is a goldmine. Keep it up everyone!",
      "I'm just getting started and already profitable.", "My withdrawals are always on time. No stress.",
      "I'm telling my whole family about this.", "Can't wait to see my portfolio in a year.",
      "The daily profit just keeps coming.", "I'm literally living my dream because of this.",
      "Another green week. Six in a row!", "There's no better feeling than passive income.",
      "I'm finally financially free. It's surreal."
    ],
    withdraw: [
      "Just withdrew $85 to my bank account. Smooth and fast!", "Withdrawal to USDT (TRC20) completed in 20 minutes. Great service.",
      "I got my money! Thanks for the reliable payouts.", "Requested withdrawal this morning, received by noon.",
      "Can't believe it's this easy. Withdrew $200 without any issues.", "For those asking – yes, withdrawals are real. I just got mine.",
      "Sent to Opay and it landed instantly. Very impressed.", "Third withdrawal this month. They never delay.",
      "Proof of payment attached. This platform is legit.", "Happy to share: just cashed out $120. Keep grinding!",
      "Withdrawing profits feels amazing. Keep pushing guys!", "I was nervous at first, but now I trust the process.",
      "Receiving money directly in my bank account – feels so good.", "Goodbye financial stress. This side hustle is paying off.",
      "I'm proof that it works. Just withdrew another chunk today.", "Withdrew $300 this morning – it hit my account before lunch.",
      "First withdrawal ever and it was flawless. So relieved!", "My Opay balance just got a nice boost. Thank you!",
      "I've made 5 withdrawals now, all fast and easy.", "Even my sceptical friend couldn't believe how fast it was.",
      "Withdrawal fees are minimal too. Very fair.", "The process is super straightforward. No hidden steps.",
      "I was worried about delays but it came through instantly.", "Receiving money while I sleep – that's the dream.",
      "Cashed out some profits to pay for my daughter's school fees.", "My rent money just came from trading profits. So grateful.",
      "Never had a withdrawal issue. Support is always helpful.", "Today's withdrawal went to my USDT wallet in minutes.",
      "I've tried other platforms, but this payout speed is unmatched.", "Just in time for the weekend. Withdrawal landed Friday morning."
    ],
    testimonial: [
      "I was skeptical at first, but now I'm a believer. Profits are real.", "Finally a platform that delivers. Withdrew my first profit today!",
      "Been following for a month, and I can honestly say it's changed my finances.", "If you're new, trust the process. I'm up 3 days in a row.",
      "Not a bot, real person. I've made consistent gains for 2 weeks.", "This community gave me the confidence to start trading. So grateful.",
      "Honest review: I've tried many groups, but this one actually works.", "Up 40% this week. I never thought I'd see results like this.",
      "I'm just a regular person, and this platform is making a difference.", "Thank you to the admins for the signals. They've been spot on.",
      "I never leave reviews, but I had to. This is the real deal.", "My family noticed the change. I can finally contribute more at home.",
      "I started with a small amount, and now I'm growing it every day.", "Never been so consistent. The signals are high quality.",
      "I'm paying off debts little by little. This is life changing.", "I was about to give up on trading until I found this.",
      "Trust me, I've been burned before. But this is different.", "The community is what makes it special. Real people sharing real results.",
      "My portfolio has never looked this green. It's remarkable.", "I was so lost, but the education here helped me turn it around.",
      "Thank you Trade Pulse AI. You've given me hope.", "I'm telling everyone I know. This is too good to keep secret.",
      "I've made more in a month than I did all of last year.", "I was scared of losing money, but the risk management tips work.",
      "No more 9‑5 stress. I'm building my own future.", "My confidence has grown so much. I'm actually trading well now.",
      "I'm a single mom, and this extra income is a lifesaver.", "I retired early thanks to the profits I made here.",
      "Words can't express how grateful I am. Bless this community.", "If I can do it, anyone can. Just follow the plan."
    ],
    advice: [
      "Always use a stop loss. Protect your capital first.", "Never risk more than 2% per trade. It's the golden rule.",
      "Don't revenge trade. Step away if you're emotional.", "Stick to your plan. Impulse trades are account killers.",
      "Patience is key. Wait for the setup, don't chase.", "Trade with the trend – it's your best friend.",
      "Backtest before going live. Save yourself unnecessary losses.", "Only 7 spots left for the premium signals this week. Don't wait.",
      "The early bird gets the best setups. London session is approaching!", "If you're unsure, stay out. Cash is also a position.",
      "Limit your screen time – overtrading is a real problem.", "Focus on a few pairs. You can't master everything at once.",
      "Use a trading journal. It will show you where you're winning and losing.", "Celebrate small wins. Consistency builds confidence.",
      "Don't let a losing day define you. Tomorrow is a new opportunity.", "Set realistic goals. Slow and steady wins the race.",
      "Keep your emotions in check. The market doesn't care about your feelings.", "Learn to read price action – indicators can lag.",
      "Understand the news, but don't let it dictate your trades.", "Position sizing is everything. Never go all in.",
      "Treat trading like a business, not a hobby.", "Keep learning. The market always has something to teach.",
      "Have a support system. Trading can be lonely sometimes.", "Know when to walk away. A fresh mind makes better decisions.",
      "Diversify your strategies, but not too much.", "Stay humble. The market can humble anyone quickly.",
      "Track your stats. What gets measured gets improved.", "Don't compare yourself to others. Your journey is unique.",
      "Invest in your education first, then your account.", "Remember why you started. Keep that vision in mind."
    ],
    nigerian: [
      "Abeg, who fit explain the minimum deposit again?", "I just cashout small money, this thing sweet o",
      "Na God dey run this platform. I dey tell you", "Una see the signal wey drop for VIP? E too correct",
      "Omo, the withdrawal hit my Opay like play", "I no fit shout, my account don turn green again",
      "Wahala no dey for this trade. Everything smooth", "Make una no dull, this market na for serious people",
      "I just dey observe, but I go soon jump in", "Naija to the world! We dey represent",
      "E go better for all of us. Just trust the process", "Shey you see that move? I nearly faint",
      "I dey happy say I join this family", "No be hype, this thing work well well",
      "Make I see the next signal, I ready to load", "Omo see gains! I don hammer",
      "I no know say trading fit sweet like this", "Na who get money dey sleep? We dey collect!",
      "I dey tell my people make dem join. E no easy to explain", "This profit wey I see so, I go buy motor soon"
    ],
    mexican: [
      "Hermano, esta señal está de locos 🔥", "Acabo de retirar sin problemas. Increíble servicio.",
      "¡Qué chulada! Otra semana en verde.", "Jaja, ya ni me preocupo. Todo automático.",
      "No manches, hoy saqué $200 en un solo trade.", "La comunidad es puro amor. Gracias a todos.",
      "Estoy aprendiendo muchísimo aquí.", "Vamos por más, México presente.",
      "Nunca había confiado en un bot, pero esto es real.", "Ya le dije a mi compadre que se meta.",
      "Puro pinche profit, compa.", "La neta, esto sí cumple.", "Ando feliz, ya pagué mis deudas.",
      "Aquí el que no arriesga no gana, pero con control.", "Mis respetos a los que diseñaron este sistema.",
      "Cada día estoy más sorprendido.", "Los gringos se lo pierden, jaja.", "Hoy me toca celebrar. Otra retirada exitosa.",
      "Mi esposa ya no me regaña, jajaja.", "Desde que empecé, todo ha cambiado para bien."
    ]
  };

  // ========== TYPO & SLANG HELPERS ==========
  const commonTypos = {
    "the":"teh","and":"adn","you":"yuo","are":"aer","that":"taht","with":"wiht","have":"hav",
    "this":"tihs","will":"wil","when":"wehn","where":"wher","there":"tehre","their":"thier",
    "profit":"proffit","signal":"singal","trade":"tarde","market":"makret","stop":"sotp",
    "loss":"lose","time":"tiem","today":"todday","good":"goood","great":"greaat","making":"makign",
    "community":"comunity","everyone":"evreyone","session":"sessoin","deposit":"depoist",
    "withdrawal":"withdrawl","support":"spport"
  };
  ...
  function applyTypos(text, persona) {
    ...
    if (commonTypos[word]) { words[idx] = Math.random() < 0.6 ? commonTypos[word] : words[idx]; return words.join(' '); }
    if (word.length > 3 && Math.random() < 0.4) {
      const pos = Math.floor(Math.random()*(word.length-1));
      const chars=word.split(''); [chars[pos],chars[pos+1]]=[chars[pos+1],chars[pos]]; words[idx]=chars.join('');
    }
    return words.join(' ');
  }
  ...
  function applySlang(text, persona) {
    ...
    const slangMap = {
      "going to":"gonna","want to":"wanna","you all":"y'all","I am":"I'm","you are":"you're",
      "cannot":"can't","do not":"don't","does not":"doesn't","let us":"let's","what is":"what's",
      "it is":"it's","that is":"that's"
    };
    for (const [key,val] of Object.entries(slangMap)) if (Math.random()<0.5 && text.includes(key)) text = text.replace(new RegExp(key,'g'),val);
    return text;
  }

  function getCountrySlang(persona) {
    if (persona.country==='Nigeria' && Math.random()<0.7) return messageCategories.nigerian[Math.floor(Math.random()*messageCategories.nigerian.length)];
    if (persona.country==='Mexico' && Math.random()<0.7) return messageCategories.mexican[Math.floor(Math.random()*messageCategories.mexican.length)];
    return null;
  }
  ...
  function getRandomMessage(persona) {
    if (persona.moderator) {
      ...
      for (let i=0;i<30;i++) { if (!recentMessagesText.has(msg)) break; msg = pool[Math.floor(Math.random()*pool.length)]; }
      recentMessagesText.add(msg); if (recentMessagesText.size>MAX_RECENT_MSGS) { const iter=recentMessagesText.values(); recentMessagesText.delete(iter.next().value); }
      return msg;
    }
    const slangMsg = getCountrySlang(persona);
    if (slangMsg) {
      let msg=slangMsg; msg=applySlang(msg,persona); msg=applyTypos(msg,persona);
    ...
    let msg=null;
    for (let i=0;i<30;i++) { const candidate=bank[Math.floor(Math.random()*bank.length)]; if (!recentMessagesText.has(candidate)) { msg=candidate; break; } }
    if (!msg) msg=bank[Math.floor(Math.random()*bank.length)];
    recentMessagesText.add(msg); if (recentMessagesText.size>MAX_RECENT_MSGS) { const iter=recentMessagesText.values(); recentMessagesText.delete(iter.next().value); }
    msg=applySlang(msg,persona); msg=applyTypos(msg,persona);
  ...
  function createChatCard(persona, message, timestamp, replyTo=null) {
    ...
    let replyHtml = '';
    if (replyTo) {
      replyHtml = `<div style="background:rgba(168,85,247,.04); border-left:3px solid #a855f7; border-radius:6px; padding:8px 10px; margin-bottom:10px; font-size:12px; color:#9ca3af;">
        <div style="font-weight:400; color:#c084fc; margin-bottom:4px;">↳ Replying to ${replyTo.persona.name}</div>
        <div style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100%;">${replyTo.message}</div>
      </div>`;
    }
    // Card with overflow visible, flex-shrink 0 to prevent clipping
    return `<div class="community-chat-card" data-msg-timestamp="${timestamp}" style="overflow:visible; display:block; min-height:fit-content; flex-shrink:0; word-break:break-word;">
      ...
      <div class="chat-msg-body" style="word-break:break-word; white-space:pre-wrap; overflow-wrap:break-word;">${replyHtml}${message}</div>
  ...
  function startReactionSimulation(cardElement) {
    ...
    const targetCounts={'👍':Math.floor(Math.random()*80)+5,'❤️':Math.floor(Math.random()*50)+2,'🔥':Math.random()>0.5?Math.floor(Math.random()*40)+1:0,'🚀':Math.random()>0.6?Math.floor(Math.random()*30)+1:0};
    let currentCounts={'👍':0,'❤️':0,'🔥':0,'🚀':0};
    setTimeout(()=>{
      if (Math.random()<0.4) { const initialEmoji=reactionEmojis[Math.floor(Math.random()*reactionEmojis.length)]; currentCounts[initialEmoji]=1; updateReactionsDisplay(reactionsDiv,currentCounts); }
  ...
  function updateReactionsDisplay(container,counts) {
    let html='';
    for (const emoji of reactionEmojis) if (counts[emoji]>0) html+=`<div class="chat-reaction">${emoji} ${counts[emoji]}</div>`;
  ...
  function ensureChatScrollable() {
    const col=document.getElementById('communityChatColumn'); if (!col) return;
    col.style.maxHeight='65vh'; col.style.overflowY='auto'; col.style.overflowX='hidden'; col.style.paddingRight='4px';
    if (window.innerWidth<400) col.style.maxHeight='55vh';
  }

  // ========== UNREAD BADGE & SCROLL ==========
  window.unreadCount = 0;
  window.latestChatCard = null;

  function updateUnreadBadge() {
    const badge = document.getElementById('unreadBadge');
    if (!badge) return;
    if (window.unreadCount > 0) {
      badge.textContent = `${window.unreadCount} new message${window.unreadCount > 1 ? 's' : ''}`;
  ...
  function scrollToLatest() {
    if (window.latestChatCard) {
      window.latestChatCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    window.unreadCount = 0;
    updateUnreadBadge();
  }
  ...
  function scrollToCard(cardElement) {
    ...
    if (!col) return;
    const threshold = 50;
    const isNearTop = col.scrollTop <= threshold;
    if (isNearTop) {
      cardElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
    } else {
      window.unreadCount++;
      window.latestChatCard = cardElement;
      updateUnreadBadge();
    }
  }
  ...
  function addRandomChatMessage() {
    ...
    const TYPING_MS_PER_CHAR = 45 + Math.random() * 20;
    const MIN_TYPING_MS = 2000;
    const MAX_TYPING_MS = 9000;
    let computedTime = message.length * TYPING_MS_PER_CHAR;
    if (Math.random() < 0.4) computedTime += randomInt(600, 2500); // thinking pause
    const typingDelay = Math.min(MAX_TYPING_MS, Math.max(MIN_TYPING_MS, computedTime));
    ...
    if (showTyping && typingDiv) {
      ...
      // 15% chance to cancel message (just hide indicator, no message)
      if (Math.random() < 0.15) {
        const cancelDelay = Math.min(typingDelay * 0.4, 3000);
        setTimeout(() => { typingDiv.style.display = 'none'; }, cancelDelay);
      } else {
        setTimeout(() => {
    ...
    function _actuallyAddMessage(persona, message) {
      ...
      const cardElement = insertChatCard(cardHtml);
      if (cardElement) {
        startReactionSimulation(cardElement);
      ...
      if (Math.random() < 0.12 && persona.traits.archetype !== 'lurker') {
        const followUpDelay = 2500 + Math.random() * 5000;
        setTimeout(() => {
          ...
          if (cardElement2) {
            startReactionSimulation(cardElement2);
            scrollToCard(cardElement2);
          }
          while (col.children.length > 80) col.removeChild(col.lastChild);
  ...
  function injectJoinNotification() {
    ...
    const cardElement = insertChatCard(joinCard.outerHTML);
    if (cardElement) scrollToCard(cardElement);
  }
  ...
  function updateOnlineMembers() {
    ...
    if (sidebarOnlineMembers) {
      sidebarOnlineMembers.innerHTML = onlinePersonas.map(p => {
        ...
        return `<div class="sidebar-member-row"><div class="sidebar-member-avatar" style="background:${p.admin?'rgba(245,158,11,.18)':'rgba(168,85,247,.18)'}">${p.initials}</div><span class="sidebar-member-name">${p.name} ${badgeHtml}</span></div>`;
      }).join('');
    }
  }
  ...
  const currencyMethods = {
    NGN: [
      { name: 'GTBank', icon: 'assets/banks/gtbank.png' }, { name: 'Access Bank', icon: 'assets/banks/access.png' },
      { name: 'Opay', icon: 'assets/banks/opay.png' }, { name: 'First Bank', icon: 'assets/banks/firstbank.png' },
      { name: 'UBA', icon: 'assets/banks/uba.png' }, { name: 'Zenith Bank', icon: 'assets/banks/zenith.png' },
      { name: 'Palmpay', icon: 'assets/banks/palmpay.png' }, { name: 'Kuda Bank', icon: 'assets/banks/kuda.png' },
      { name: 'Wema Bank', icon: 'assets/banks/wema.png' }
    ],
    USD: [
      { name: 'Bank of America', icon: 'assets/banks/boa.png' }, { name: 'Chase', icon: 'assets/banks/chase.png' },
      { name: 'Wells Fargo', icon: 'assets/banks/wells_fargo.png' }, { name: 'Citibank', icon: 'assets/banks/citibank.png' },
      { name: 'PayPal', icon: 'assets/banks/paypal.png' }, { name: 'Cash App', icon: 'assets/banks/cashapp.png' },
      { name: 'Venmo', icon: 'assets/banks/venmo.png' }, { name: 'Zelle', icon: 'assets/banks/zelle.png' },
      { name: 'USAA', icon: 'assets/banks/usaa.png' }
    ],
    EUR: [
      { name: 'Deutsche Bank', icon: 'assets/banks/deutsche.png' }, { name: 'BNP Paribas', icon: 'assets/banks/bnp_paribas.png' },
      { name: 'ING', icon: 'assets/banks/ing.png' }, { name: 'Santander', icon: 'assets/banks/santander.png' },
      { name: 'Revolut', icon: 'assets/banks/revolut.png' }, { name: 'N26', icon: 'assets/banks/n26.png' },
      { name: 'Bunq', icon: 'assets/banks/bunq.png' }, { name: 'Commerzbank', icon: 'assets/banks/commerzbank.png' },
      { name: 'Vivid', icon: 'assets/banks/vivid.png' }
    ],
    GBP: [
      { name: 'Barclays', icon: 'assets/banks/barclays.png' }, { name: 'HSBC UK', icon: 'assets/banks/hsbc_uk.png' },
      { name: 'Lloyds', icon: 'assets/banks/lloyds.png' }, { name: 'NatWest', icon: 'assets/banks/natwest.png' },
      { name: 'Monzo', icon: 'assets/banks/monzo.png' }, { name: 'Starling', icon: 'assets/banks/starling.png' },
      { name: 'Revolut UK', icon: 'assets/banks/revolut_uk.png' }, { name: 'Metro Bank', icon: 'assets/banks/metro.png' },
      { name: 'Halifax', icon: 'assets/banks/halifax.png' }
    ],
    USDT: [
      { name: 'Binance Pay', icon: 'assets/banks/binance.png' }, { name: 'Bybit', icon: 'assets/banks/bybit.png' },
      { name: 'OKX', icon: 'assets/banks/okx.png' }, { name: 'Coinbase', icon: 'assets/banks/coinbase.png' },
      { name: 'Kraken', icon: 'assets/banks/kraken.png' }, { name: 'Trust Wallet', icon: 'assets/banks/trustwallet.png' },
      { name: 'MetaMask', icon: 'assets/banks/metamask.png' }, { name: 'KuCoin', icon: 'assets/banks/kucoin.png' },
      { name: 'Huobi', icon: 'assets/banks/huobi.png' }
    ]
  };

  const currencySymbols = { NGN: '₦', USD: '$', EUR: '€', GBP: '£', USDT: '₮' };
  const toUSDT = { USDT: 1, USD: 1, EUR: 1.08, GBP: 1.27, NGN: 0.000667 };
  ...
  function createWithdrawal(persona, minutesAgo, currency) {
    ...
    return {
      id: Date.now() + Math.random(), maskedName: maskName(persona.name), initials: persona.initials,
      avatarUrl: persona.avatar, isFallback: persona.isFallback, amount, currency,
      methodName: method.name, methodIcon: method.icon, time: date, status: 'completed'
    };
  }
  ...
  currencies.forEach(cur => {
    for (let i = 0; i < 50; i++) {
      ...
      allWithdrawals.push(createWithdrawal(persona, Math.floor(Math.random() * 1440) + 1, cur));
    }
  });
  allWithdrawals.forEach(w => { if (Math.random() < 0.1) w.status = 'processing'; });
  allWithdrawals.sort((a, b) => b.time - a.time);

  let inlineLoadedCount = 6, inlineActiveFilter = 'all';
  const whiteBgExceptions = ['Binance Pay', 'Bybit', 'Cash App', 'Vivid', 'USAA'];
  function methodNeedsWhiteBg(name) { return !whiteBgExceptions.includes(name); }
  ...
  function updateSidebarWithdrawals() {
    ...
    feed.innerHTML = filtered.map(w => {
      ...
      return `<div class="live-withdrawal-row">
        <div class="live-wd-check">${w.status === 'processing' ? '<div style="width:24px;height:24px;border:2px solid #f59e0b;border-radius:50%;border-top-color:transparent;animation:spin 1s linear infinite;"></div>' : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="m5 13 4 4L19 7"/></svg>'}</div>
        <div class="live-wd-info"><div class="live-wd-name">${w.maskedName}</div><div class="live-wd-loc"><img src="${w.methodIcon}" alt="" style="width:16px;height:16px;vertical-align:middle;border-radius:2px; ${bgStyle}" onerror="this.style.display='none'"> ${w.methodName}</div></div>
        <div><div class="live-wd-amount">${formatAmount(w.amount, w.currency)}</div><div class="live-wd-time">${timeAgo(w.time)}</div></div>
  ...
  function buildWinsProofsContent() {
    ...
    container.innerHTML = `
      <div class="section" style="margin-top:0;">
        ...
        <div style="text-align:center; font-size:11px; color:#6b7280; margin-bottom:8px;">All withdrawals are verified and secured by Trade Pulse AI</div>
      </div>`;
    document.getElementById('winsProofsBackBtn')?.addEventListener('click', () => { switchToGeneralChat(); });
  }
  ...
  function updateHeroAvatarStackInline() {
    ...
    stack.innerHTML = heroAvatars.map((h, i) => `
      ...
      <div style="width:36px; height:36px; border-radius:50%; border:2px solid #0a1020; background:linear-gradient(135deg, #a855f7, #6d28d9); margin-left:-10px; z-index:2; position:relative; display:grid; place-items:center; font-size:12px; font-weight:700; color:white;">+128</div>`;
  }
  ...
  function renderFilterTabsInline() {
    ...
    document.querySelectorAll('.liveFilterTabInline').forEach(tab => {
      tab.addEventListener('click', () => {
        document.querySelectorAll('.liveFilterTabInline').forEach(t => { t.style.background = 'rgba(255,255,255,.04)'; t.style.color = '#9ca3af'; t.classList.remove('active'); });
        tab.style.background = 'rgba(168,85,247,.2)'; tab.style.color = '#c084fc'; tab.classList.add('active');
  ...
  function renderLiveWithdrawalListInline() {
    ...
    list.innerHTML = visible.map(w => {
      ...
      return `<div style="display:flex; align-items:center; gap:10px; padding:12px 0; border-bottom:1px solid rgba(255,255,255,.04);">
        ${avatarHTML(w)}
        <div style="flex:1; min-width:0;"><div style="font-size:13px; font-weight:600; max-width:100px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${w.maskedName}</div><div style="font-size:11px; color:#9ca3af;"><img src="${w.methodIcon}" alt="" style="width:16px;height:16px;vertical-align:middle;border-radius:2px;margin-right:4px; ${bgStyle}" onerror="this.style.display='none'"> ${w.methodName}</div></div>
        <div style="text-align:right;"><div style="font-size:14px; font-weight:700; color:#4ade80;">${formatAmount(w.amount, w.currency)}</div><div style="font-size:10px; color:#6b7280;">${timeAgo(w.time)}</div></div>
        <span style="background:${w.status==='processing'?'rgba(245,158,11,.2)':'rgba(34,197,94,.1)'}; color:${w.status==='processing'?'#f59e0b':'#22c55e'}; padding:2px 8px; border-radius:999px; font-size:10px; font-weight:600;">${w.status==='processing'?'Processing':'Completed'}</span>
      </div>`;
    }).join('');

    if (inlineLoadedCount >= filtered.length) loadBtn.style.display = 'none';
  ...
  function getUserAnnouncementDates() {
    ...
    if (user.announcementDates && Array.isArray(user.announcementDates) && user.announcementDates.length === 12) {
      if (user.announcementDates.every(d => typeof d.postedStr === 'string' && d.postedStr.length > 0)) return user.announcementDates;
    }
    const now = new Date();
    let firstVisit;
    ...
    for (let i = 0; i < 12; i++) {
      if (i === 0) {
        ...
        dates.push({ id: 1, postedDate, closingDate: nextFriday, postedStr: postedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ' · ' + postedDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }) });
      } else {
        const postedDate = new Date(now);
        postedDate.setDate(postedDate.getDate() - Math.floor(Math.random() * 7 + 1));
        dates.push({ id: i+1, postedDate, closingDate: null, postedStr: postedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ' · ' + postedDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }) });
      }
    }
  ...
  function renderAnnouncements() {
    ...
    if (!container) {
      ...
      if (tabsDiv) tabsDiv.parentNode.insertBefore(container, tabsDiv.nextSibling);
      else document.getElementById('communityView').appendChild(container);
    }
    const userDates = getUserAnnouncementDates() || [];
    const getDateForId = (id) => { const found = userDates.find(d => d.id === id); return found && found.postedStr ? found.postedStr : 'Unknown date'; };
    ...
    container.innerHTML = `
      <div class="section" style="margin-top:0;">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;"><div style="font-size:17px; font-weight:700;">📢 Announcements</div><span style="background:rgba(59,130,246,.15); color:#60a5fa; font-size:12px; padding:4px 10px; border-radius:999px;">12 New</span></div>
        ${announcementsTemplate.map(a => {
          ...
          if (a.id === 1) {
            ...
            if (closingDateItem && closingDateItem.closingDate) {
              ...
              if (!isNaN(closingDate.getTime())) desc = a.description.replace(/closes on .+$/, `closes on ${closingDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}.`);
            }
          }
          return `<div style="background:rgba(11,16,32,.9); border-radius:16px; padding:14px; margin-bottom:12px; border:1px solid rgba(255,255,255,.05);">
            <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">${adminAvatarHtml}<div><div style="font-weight:600; color:#fff;">${a.title}</div><div style="font-size:12px; color:#9ca3af;">Admin · ${getDateForId(a.id)}</div></div></div>
            ...
            <div style="display:flex; gap:8px; flex-wrap:wrap;">${Object.entries(a.reactions).map(([emoji, count]) => `<span style="background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.08); border-radius:999px; padding:4px 10px; font-size:12px; color:#d1d5db;">${emoji} ${count}</span>`).join('')}</div>
          </div>`;
        }).join('')}
      </div>`;
  }
  ...
  function switchToWinsProofs() {
    ...
    inlineLoadedCount = 6;
    renderLiveWithdrawalListInline();
    document.getElementById('loadMoreWithdrawalsBtnInline')?.addEventListener('click', () => { inlineLoadedCount += 10; renderLiveWithdrawalListInline(); });
  }
  ...
  function injectNewLiveWithdrawal() {
    ...
    setTimeout(() => {
      ...
      if (document.getElementById('communityView')?.classList.contains('active')) {
        ...
        if (document.getElementById('winsProofsContent')?.style.display !== 'none') renderLiveWithdrawalListInline();
      }
  ...
  function startCommunitySimulation() {
    try {
      ...
      badge.style.boxShadow = '0 4px 12px rgba(168,85,247,.4)';
      badge.addEventListener('click', scrollToLatest);
      col.parentNode.insertBefore(badge, col.nextSibling);
