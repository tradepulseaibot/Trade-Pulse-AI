// ===== Trade Pulse AI – community.js (FULL – typing indicator debugged, forced test, NO debug panel) =====
(function() {
  ...
  function addRandomChatMessage() {
    ...
    function _actuallyAddMessage(persona, message) {
      ...
      // Insert after the typing indicator to keep it as first child
      ...
      const cardElement = indicator ? indicator.nextElementSibling : col.firstElementChild;
      ...
      if (Math.random() < 0.15 && persona.traits.archetype !== 'lurker') {
        ...
        setTimeout(() => {
          ...
          if (ind) {
            ind.insertAdjacentHTML('afterend', cardHtml2);
          } else {
            col.insertAdjacentHTML('afterbegin', cardHtml2);
          }
          
          const cardElement2 = ind ? ind.nextElementSibling : col.firstElementChild;
  ...
  function preloadChatMessages() {
    ...
    // Clear only chat messages, preserve typing indicator
    const indicator = document.getElementById('typingIndicator');
    if (indicator) {
      // Remove all children that are NOT the typing indicator
      while (col.firstChild) {
        if (col.firstChild.id === 'typingIndicator') {
          // Move to next sibling if indicator is first; but we can just skip it
          // Actually we need to remove only non-indicator elements.
          // Since indicator is first child, we can remove all others after it.
          // Let's just loop through all children and remove non-indicator.
          // Simpler: we know indicator is first child, so remove everything after it.
          ...
          break; // indicator is first, no need to check further
        } else {
          col.removeChild(col.firstChild);
        }
      }
    } else {
      // No indicator, fallback to clearing everything
      col.innerHTML = '';
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      const ind = document.getElementById('typingIndicator');
      if (ind) {
        ind.insertAdjacentHTML('afterend', cardHtml);
      } else {
        col.insertAdjacentHTML('beforeend', cardHtml);
      }
      
      const cardElement = ind ? ind.nextElementSibling : col.lastElementChild;
      if (cardElement) startReactionSimulation(cardElement);
      trackRecentMessage(persona, message);
    }
  }
  ...
  function injectJoinNotification() {
    ...
    const indicator = document.getElementById('typingIndicator');
    if (indicator) {
      indicator.insertAdjacentHTML('afterend', joinCard.outerHTML);
    } else {
  ...
  function startCommunitySimulation() {
    try {
      ...
      // Create typing indicator (styled to stand out) before any messages
