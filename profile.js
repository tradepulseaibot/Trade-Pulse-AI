...
(function() {
  ...
  function saveCurrentUser(user) {
    ...
    renderProfile();
    forceUpdateDrawerAvatar(); // Immediate drawer avatar update
  }
  ...
  /* ---------- Immediate Drawer Avatar Update ---------- */
  function forceUpdateDrawerAvatar() {
    const user = getCurrentUser();
    if (!user) return;
    const avatar = document.getElementById('drawerAvatar');
    if (!avatar) return;

    // Preserve file input and camera icon
    const fileInput = avatar.querySelector('#avatarInput');
    const cameraIcon = avatar.querySelector('.camera-icon');
    avatar.innerHTML = '';
    if (fileInput) avatar.appendChild(fileInput);
    if (cameraIcon) avatar.appendChild(cameraIcon);
